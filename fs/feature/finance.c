...
int pay_player(int amount)
{
  ...
  if(amount >= 2100000000 || amount < 0){
    tell_object(this_object(),"要給你的錢太多了，結果錢莊倒店，付不出錢!!\n");
    tell_object(this_object(),"把過程告訴大神，請大神解決吧!!!\n");
    return 0;
  }
...
int pay_money(int amount)
{
  ...
  if(amount >= 2100000000 || amount < 0){
    tell_object(this_object(),"你要給的錢太多了，錢莊無法受理，你的錢沒辦法交出去!!\n");
    tell_object(this_object(),"把過程告訴大神，請大神解決吧!!!\n");
    return 0;
  }
  total = 0;
 ...
 // if( diamond ) total += diamond->value();
...
//先把身上的錢都整理過
  pay_player(0);
//再查amount(東西的價值)，看看要用那一層的換算
//出來的值,value_level是層數,amount是價值
  value_level=0;//範圍0 ~ 4,0==coin,1==silver,2==gold,3==cash,4==diamond
  while(!(amount % 100))
  {
    amount/=100;
    value_level++;
...
/*
basic是基值,利用value_level去計算,因為不曉得次方符號,所以用迴圈做
...
*/
  ...
  do
  {
    ...
    else
    {
      ...
      /*
      依照basic的不同來考慮最大值
      因為已經整理過玩家的錢了, 所以考慮diamond就可以
      最大值,是用basic*20來算
      */
      if(mylevel==sizeof(money_name)-1){//用錢的最高層去算
  ...
  if(total < 0)
    write("你身上的錢太多了, 使得系統判斷錯誤, 建議先將身上的diamond存入bank!!\n");
