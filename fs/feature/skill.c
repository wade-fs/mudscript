	...
	if( !weak_mode && learned[skill] > (skills[skill]+3)*(skills[skill]+3) ) {
		...
		SKILL_D(skill)->skill_improved(this_object());
                ...
                if((this_object()->query_skill(skill,1)%5) == 0)
                 if(id != "guest" || this_object()->query_skill(skill,1) > 30)
                  if(userp(this_object())) log_file(""+file,sprintf("%s 的%s升到 %d級 於 %s\n",id,to_chinese(skill),this_object()->query_skill(skill,1),ctime(time())));
	}
}
