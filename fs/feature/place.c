...
int set_place(object room,int num)
{
    ...
    if(wizardp(this_object()))
      if(sizeof(places) >= MAX_WIZ_SET)
        return notify_fail("你設定的place太多了, 請先刪除一些不必要的place!!\n");
    else
    ...
    tell_object(this_object(),"你設定這裡為你的傳送點。\n");
    return 1;
}
int delete_place(int num)
{
    ...
    if(!mapp(places))
      return notify_fail("你並沒有設定place!!\n");
    index=sprintf("%d",num);
    if(undefinedp(places[index]))
    {
      txt=sprintf("你並沒有設定編號第 %d 號的places!!\n",num+1);
      return notify_fail(txt);
    }
    txt=sprintf("刪除第 %d 號places:\n",num+1);
    prop=index+"/name";
