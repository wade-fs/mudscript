...
{
  if( ob==this_object() )
    error("set_leader: "+ob->short()+"企圖設自己為 leader.\n");
  leader = ob;
}
...
int follow_me(object ob, string dir)
{
  ...
  // 死人是不會走路的, 當然, 自己也不必跟著自己走, 否則會陷入無窮迴圈
  // 被 gage 的人也不會跟來, 以免  pker 利用 follow K 玩家
  if ( !living(me) || ob==me || ob->gage(me))
  {
    return 0;
  }

  // 假如是 leader 走路, 我們應該是馬上跟著走的
  if (ob == leader)
