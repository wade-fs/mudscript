...
void create()
{
set_name(HIW"九天仙龍"NOR, ({ "god dragon","dragon" }) );
...
int do_ascend()
{
        ...
        if (name != "dunga")
           return notify_fail("你不是這座騎的主人(dunga)無法騎他!!!\n");

        if ( me->query_temp("marks/ascend") )
               return notify_fail("你無法騎在別人的仙龍身上呀。\n");

  if(me->query("id","dunga")) {

message_vision("只見雲霧慢慢散去,天空降下一道佛光,$N沐浴在佛光下,不知覺的已駕馭著"+this_object()->name()+"。\n",me);
...
message_vision("$N雙手合十 嘴中念著梵語,只見"+this_object()->name()+"漸漸隱沒於雲霧中。\n",me);
        me->delete_temp("marks/ascend");   
       me->delete_temp("fs_net/dunga");
