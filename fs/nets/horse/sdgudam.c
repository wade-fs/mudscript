...
void create()
{
set_name(HIC + "嘯天犬" + NOR, ({ "fly dog","dog" }) );
set("long","這是生存在仙界九天之上的仙犬,是巫師們賞賜給人們的\n似乎還能騎上去唷(ride)");
set("no_drop", 1);
set("no_auc", 1);
set("no_sell", 1);
set("unit","隻");
...
int do_ascend()
{
        ...
        if (name != "sdgudam")
           return notify_fail("你不是這座騎的主人(sdgudam)無法騎他!!!\n");

        if ( me->query_temp("marks/ascend") )
               return notify_fail("你無法騎在別人的座騎身上呀。\n");

        message_vision("$N仰天長嘯跳向天空,頓時天空一聲怒吼,只見$N已跨在"+this_object()->name()+"的身上。\n",me);
...
message_vision("$N一個後空翻從"+this_object()->name()+"的背上跳了下來,瞬間"+this_object()->name()+"消失無蹤。\n",me);
