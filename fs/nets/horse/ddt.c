...
void create()
{
set_name(HIR + "超級壞馬" + NOR, ({ "superbad-horse","horse" }) );
...
int do_ascend()
{
        ...
        if (name != "ddt")
           return notify_fail("你不是這座騎的主人(ddt)無法騎他!!!\n");
...
message_vision("在莊家一陣叫罵之中,"+this_object()->name()+"從賭場裡衝出來\n$N一個翻身落在"+this_object()->name()+"的背上。\n",me);
...
message_vision("$N縱身一跳,"+this_object()->name()+"已然消失無蹤。\n",me);
