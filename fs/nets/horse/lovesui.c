...
void create()
{
set_name(BLU + "風殘" + HIW + "雪影" + NOR, ({ "shadow snow","snow" }) );
...
int do_ascend()
{
        ...
        if (name != "lovesui")
           return notify_fail("你不是這座騎的主人(lovesui)無法騎他!!!\n");
        if ( me->query_temp("marks/ascend") )
               return notify_fail("你無法騎在別人的仙龍身上呀。\n");

message_vision("只見$N一瞬間消失於你眼前,以迅雷不及掩耳的速度,$N已駕馭在"+this_object()->name()+"的背上。\n",me);
...
message_vision("$N一個翻身,口中喃喃地念著咒語,"+this_object()->name()+"竟漸漸散化於空氣中。\n",me);
