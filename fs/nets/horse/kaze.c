...
void create()
{
set_name(HIC + "聖獸" + HIR + "牙炎" + NOR, ({ "god dragon ","dragon" }) );
set("long","傳說中操縱火焰的聖獸，只有他所承認的繼承人可以召喚他，\n"
           "擁有不可估計的神秘力量，似乎還能騎上去唷(ride)\n");
set("no_drop", 1);
set("no_auc", 1);
set("no_sell", 1);
set("unit","隻");
...
int do_ascend()
{
        ...
        if (name != "kaze")
           return notify_fail("你不是這座騎的主人(kaze)無法騎他!!!\n");
        if ( me->query_temp("marks/ascend") )
               return notify_fail("你無法騎在別人的仙龍身上呀。\n");

message_vision("在火焰的魔法陣中，"+this_object()->name()+"伴隨著熊熊烈火降臨。$N身形一晃\n"
               "立刻出現在"+this_object()->name()+"的身上，全身充滿非凡的氣勢。\n",me);
...
message_vision(""+this_object()->name()+"在一聲清嘯後消失在次元的夾縫中，現場只留下熊熊的天火。\n",me);
