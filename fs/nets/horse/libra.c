...
void create()
{
set_name(HIC + "傲雪狂龍" + NOR, ({ "crazy dragon","dragon" }) );
...
int do_ascend()
{
   ...
   if (name != "libra" && name != "libraa")
           return notify_fail("你不是這座騎的主人(Libra)無法騎他!!!\n");

        if ( me->query_temp("marks/ascend") )
               return notify_fail("你無法騎在別人的仙龍身上呀。\n");

message_vision("$N跳上了傲雲狂龍,雲霧把$N的身體包覆起來.使你產生了無比的信心。\n",me);
...
message_vision("$N跳下雲傲狂龍使雲傲狂龍化為一團雲霧...\n",me);
...
{
      message_vision(HIW+"道道閃光從傲雪狂龍口中疾吐而出,正朝著$N迅速直劈而去!!\n"+NOR,enemy[i]);
  ...
  COMBAT_D->report_status(enemy[i], 1);
      ...
      message_vision(HIW+"道道閃光傲雪狂龍的口中疾吐而出,正朝著$N迅速直襲而去!!\n"+NOR,enemy[i]);
