...
void create()
{
set_name(HIC + "冷血" + BLU + "專用坐騎" + NOR, ({ "pony's horse","horse" }) );
...
message_vision("$N口中喃喃唸著，瞬間天空閃出一道火光，只見$N已坐在"+this_object()->name()+"上。\n",me);
        me->set_temp("marks/ascend",1);
if(me->query("class")=="blademan")  me->add_temp("apply/blade",30);
if(me->query("class")=="fighter")   me->add_temp("apply/unarmed",30);
if(me->query("class")=="swordsman") me->add_temp("apply/sword",30);
if(me->query("class")=="scholar")   me->add_temp("apply/stabber",30);
if(me->query("class")=="bandit")    me->add_temp("apply/unarmed",30);
...
message_vision("$N大叫一聲「徹」，瞬間"+this_object()->name()+"已然消失無蹤。\n",me);
