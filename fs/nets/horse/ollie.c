...
void create()
{
set_name(HIR + "赤血" + HIY + "妖龍" + NOR, ({ "blood dragon","dragon" }) );
...
int do_ascend()
{
        ...
        if (name != "ollie")
           return notify_fail("你不是這座騎的主人(ollie)無法騎他!!!\n");
        if ( me->query_temp("marks/ascend") )
               return notify_fail("你無法騎在別人的座騎身上呀。\n");

        message_vision("$N大喝一聲只見一道血紅的光影飛至，$N迅速往"+this_object()->name()+"一跨騎了上去。\n",me);
...
message_vision("$N一個後旋翻從"+this_object()->name()+"的背上摔了下來。\n",me);
