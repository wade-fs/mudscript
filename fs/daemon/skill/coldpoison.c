...
int make(object me,string name)
{
	...
	string pstr="";
	if(!make[name])
		return notify_fail("天冰奇毒中沒有這種毒。\n");
	if(me->query_skill("coldpoison",1)<make_need_lv[name])
		return  notify_fail("你的天冰奇毒程度不夠﹐無法練製這種毒。\n");
	dict=make[name];
	...
	for(i=0;i<5;i++)
	{
	  ...
	  me->improve_skill("coldpoison",random(me->query_int()/2 +make_improve[name]*me->query_skill("poison",1)/10)+4*make_improve[name],1);
	return 1;
}
