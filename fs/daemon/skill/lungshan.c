...
// 配合fs mob 強化 加上force額外傷害 by unfinished 2004/2/9
//將receive_damage的攻擊參數補上攻擊方之參數 by blazakira 2011/4/25
...
mapping query_action(object me, object  weapon)
{
  ...
  if(me->query("combat_exp")>1500000) {
    if(!me->is_busy() && userp(me) && 20>random(100) && me->query("family/family_name")=="瀧山派") {
      for(a=0;a<i;a++) {
        if(!enemy[a]) continue;
...
LONG+NOR,me);
        for(i=0;i<3;i++)
        {
          ...
          victim->receive_damage("kee",100+me->query_skill("force")/2,me);
...
void fire1(object me, object victim, object  weapon, int damage)
{
  ...
  if (skill_level > 80)
  {
    ...
    victim->receive_damage("kee",(100+me->query_skill("force")+2*sp_value),me);
    me->add("force",-100);
    victim->apply_condition("burn",random(10)+1);
    COMBAT_D->report_status(victim);
  }
...
void fire2(object me, object victim, object  weapon, int damage)
{
  ...
  if (skill_level > 90)
  {
    ...
    victim->receive_damage("kee",(150+me->query_skill("force")+2*sp_value),me);
    me->add("force",-150);
...
void fire3(object me, object victim, object  weapon, int damage)
{
  ...
  if (skill_level >= 100)
  {
    ...
    victim->receive_damage("kee",(200+me->query_skill("force")+2*sp_value),me);
    me->add("force",-200);
...
void berserk(object me, object victim, object  weapon, int damage)
{
  int bovalue,sklv,ko,bound=70,i;
  sklv=(int)me->query_skill("lungshan",1)/10;
  if((me->query("id")=="cgy"||me->query("id")=="master jen")||(80>random(100)&&me->query("env/瀧山連擊")&&me->query("family/family_name")=="瀧山派")) {
    for(i=0;i<sklv;i++)
    {
      switch(i) {
        case 0:
...
{
  // 提高命中率 by swy
  if(80>random(100) && me->query("super_fire",1))
  {
    message_vision(HIC"$n被瀧山拳法中所蘊涵的山川流水之氣息所侵，使$n吐了一口鮮血。\n"NOR,me,victim);
    victim->receive_damage("kee",me->query_skill("force")+250,me);
    me->add("force",-50);
    COMBAT_D->report_status(victim);
  }
}
...
void berserk2(object me, object victim, object  weapon, int damage)
{
  ...
  {
    message_vision( HIR"$N臉色一變,全身散出炙熱無比的真氣,雙手如旋渦般使出瀧山絕學之禁斷技!!!\n\n"HIY
      "      ☆★『"HIW"火～海～焚～天  "HIB"波～濤～洶～湧"HIY"』★☆\n"HIR
      "源源不斷的炙熱氣勁如大海浪般,一層又一層的向$n狂捲而去,$n被包圍在熱浪之中\n"
      "$n只覺巨大的炙熱氣勁入侵全身經脈無法凝氣,感覺內力不斷地在流失之中\n"NOR
      ,me,victim);
    victim->receive_damage("kee",me->query_skill("force")+500,me);
