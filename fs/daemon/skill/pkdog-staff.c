...
//將receive_damage receive_wound的攻擊參數補上攻擊方之參數 by blazakira 2011/7/2
//將天賦的判斷採新系統設定 因此基本上 作為機率的部份 就依照原始天賦 作為傷害值的部份就看累計天賦(whatup允許) by blazakira 2011/7/23
...
//12
  ([
    "action" :
    "$N將手上的$w,看拙實巧直直剌向$n的$l，正是一招"HIW"『若隱若現』。"NOR,
    "damage" : 150,
    "force" : 75,
    "damage_type" : "擊傷",
    "dodge" : -15,
...
mapping query_action(object me, object weapon)
{
     ...
     && me->query("env/打狗連擊")=="YES")
  {
    ...
    for(i=0;i<random(k)+1;i++)
    {
      for(j=0;j<sizeof(enemy);j++)
      {
        ...
        if( random(me->query("combat_exp")) > random((enemy[j]->query("combat_exp"))/3))
        {
          ...
          kee = kee/50;
          if(kee > 2000) kee = 2000;
          enemy[j]->receive_wound("kee",kee,me);
    ...
    if(random(me->query_cps()) < random(10))
    {
      me->receive_damage("sen",10,me);
...
void two (object me, object target, object  weapon, int damage)
{
  ...
  if (me->query("beggar/got_secret")==1 )
  {
    ...
    if (random(corr+karr) > (ecor+ekar)/2)
    {
      ...
      enemy[j]->receive_wound("gin",random(3)*30+120,me);
          ...
          switch(i)
          {
            case 0:
              if (random(2) == 1)
              {
                ...
                enemy[j]->receive_wound("kee",100+random(mks)*20,me);
                COMBAT_D->report_status(enemy[j]);
              }
              else
              {
            ...
            case 1:
              if (random(2) == 1)
              {
                ...
                enemy[j]->receive_wound("sen",50+random(mks)*20,me);
                COMBAT_D->report_status(enemy[j]);
              }
              else
              {
            ...
            case 4:
              if (random(2) == 1)
              {
                ...
                enemy[j]->receive_wound("kee",200+random(mks)*20,me);
                enemy[j]->receive_damage("gin",50+random(mks)*10,me);
                enemy[j]->receive_damage("sen",50+random(mks)*10,me);
                COMBAT_D->report_status(enemy[j]);
              }
              else
              {
     ...
     && me->query("combat_exp") >4000000 && random(spi) > 15)
  {
    ...
    for (i=0;i<mks;i++)
    {
      ...
      if(random(me->query("combat_exp")) > random(enemy->query("combat_exp"))/10)
      {
        ...
        if(se > 400) se = 400;
        enemy->receive_wound("kee",ke,me);
        enemy->receive_damage("gin",gi,me);
        enemy->receive_damage("sen",se,me);
        me->add("force",-30);
...
void peratt (object me, object target, object  weapon, int damage)
{
  ...
  x=random(3);
  if(me->query("gender")=="女性" && me->query("combat_exp")>9000000)
  {
      ...
      switch(i)
      {
        case 0:
          ...
          if ((int)me->query_cps() > random((int)target->query_cps()))
          {
            ...
            target->start_busy(1+random(per/10));
            COMBAT_D->report_status(target);
          }
          else
          {
        ...
        case 3:
          ...
          if((int)(me->query_per()+me->query_spi())> random(20))
          {
            ...
            target->receive_wound("gin",per*5,me);
            target->add("force",-(int)(target->query("force")/50));
            me->add("force",(int)(target->query("force")/50));
            COMBAT_D->report_status(target);
          }
          else
          {
        ...
        case 4:
          ...
          if(random(30)<per)
          {
            ...
            target->receive_wound("kee",(int)(target->query("eff_kee")/50),me); // 算法過強,稍做調整 by obs
            COMBAT_D->report_status(target);
          }
          else
          {
...
void sevenkill(object me, object weapon, int damage)
{
  ...
  j=sizeof(enemy);
  l=sizeof(enemy);
  if(!enemy) return; //先使用這試試能否避免下行敵人不存在所造成的判斷錯誤 by blazakira
  ...
  cor=me->query_cor();
//打npc會較痛哦
//算了一下，打玩家若以(20000+5000)/1000 * 6 + 10*20 = 25*6+200=350
  if(userp(target))
    damage = random(((mks+bell)/1000)* (limit/20)) +
      me->query("force_factor",1) * (int)me->query("combat_exp")/500000;
  else  
    damage = ((mks+bell)/600)* (limit/10)+
               me->query("force_factor",1) * (int)me->query("combat_exp")/500000; //about 710
      ...
      switch( i )
      {
        ...
        "HIW"　╰═══╯  ╰════╯　╰╯　╰╩╯　╯╯╯╯╰"NOR+HIW"
          ...
          while(l--)
          {
            if(cor > random(40)){
              message_vision(HIR"$N殺意凌厲，數以萬千針般似的殺氣在$n穿出了數百針孔。\n"NOR,me,enemy[l]);
              enemy[l]->receive_wound("kee",damage,me);
              target->start_busy(2);
