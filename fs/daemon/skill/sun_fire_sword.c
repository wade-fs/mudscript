...
//1
  ([
    "action"     : HIC + "$N" + HIC + "收光為暈斂剛成柔，使出了仙劍禁斷之連陽七訣第二式" + HIY + "『殘陽』" + HIC + "，瞬間身化殘影落步，劍尖分指$n" + HIC + "$l。" + NOR,
    "dodge"      : -100,
    "parry"      : -100,
    "damage"     : 380,
    "force"      : 330,
...
//2
  ([
    "action"     : HIC + "$N" + HIC + "步若游光使出仙劍禁斷之連陽七訣第四式" + HIY + "『暖陽』" + HIC + "，劍氣烘燃，一流宏大的劍壓曲地逼近$n" + HIC + "。" + NOR,
    "dodge"      : -100,
    "parry"      : -100,
    "damage"     : 350,
    "force"      : 340,
...
//3
  ([
    "action"     : HIC + "$N" + HIC + "飛身高躍倏然使出了仙劍禁斷之連陽七訣第三式" + HIY + "『豔陽』" + HIC + "，劍光如浩日輝煌，筆直落在$n" + HIC + "眉心。" + NOR,
    "dodge"      : -100,
    "parry"      : -100,
    "damage"     : 400,
    "force"      : 350,
...
//4
  ([
    "action"     : HIC + "$N" + HIC + "撫劍如靜，心若止水劍光瀲豔，仙劍禁斷之連陽七訣第五式" + HIY + "『冬陽』" + HIC + "泛起波波殺意如刃向$n" + HIC + "繞流而去。" + NOR,
    "dodge"      : -100,
    "parry"      : -100,
    "damage"     : 420,
    "force"      : 370,
...
//5
  ([
    "action"     : HIC + "$N" + HIC + "手裡的$w" + HIC + "泛出陣陣如暮色般的劍氣，仙劍禁斷之連陽七訣第七式" + HIY + "『夕陽』" + HIC + "猶如霞光滿天盡落$n" + HIC + "身上。" + NOR,
    "dodge"      : -100,
    "parry"      : -100,
    "damage"     : 430,
    "force"      : 380,
...
//6
  ([
    "action"     : HIC + "$N" + HIC + "峰迴路轉內勁猛然上竄使出仙劍禁斷之連陽七訣第一式" + HIY + "『烈陽』" + HIC + "，劍光如烈陽狂燄直奔$n" + HIC + "心口。" + NOR,
    "dodge"      : -100,
    "parry"      : -100,
    "damage"     : 440,
    "force"      : 390,
...
//7
  ([
    "action"     : HIC + "$N" + HIC + "將$w" + HIC + "收鞘而入，聚精凝神使出仙劍禁斷之連陽七訣第六式" + HIY + "『朝陽』" + HIC + "，只見劍光如晨光乍現劃向$n" + HIC + "。" + NOR,
    "parry"      : -100,
    "dodge"      : -100,
    "damage"     : 450,
    "force"      : 400,
...
mapping query_action(object me, object *weapon)
{
  ...
  limit= skill_level;
  if(me->query("family/family_name")!="仙劍派"  || me->query("killyao")==1)
    return action[7];
  if (limit < 15)
...
一心千切，同一把劍，舞的卻是不同的招式，招招朝著$n的要穴竄去。
    ...
    if( random(me->query_cps()) < 4 ) {
        me->add("force",-100);
        message_vision("\n$N使完" + HIY + "『殘陽千意』" + NOR + "後，由於定力不足，體內真氣一時無以為繼。\n",me);
        me->start_busy(1);
    }
  }
}
...
void sp_attack1(object me, object victim, object  weapon, int damage)
{
  ...
  message_vision(HIW + "
劍壓流動，隨著$N的心念越發強烈，$n" + HIW + "無從反動，完完全全
...
void sp_attack3(object me, object victim, object  weapon, int damage)
{
  ...
  message_vision(HIW + "
$N" + HIW + "內勁頓轉使出連陽唯一陰寒之招，瞬時間一股寒氣籠罩身旁，只見
劍上薄冰漸起幻成冰柱，$N" + HIW + "愈使愈快，宛若洶湧巨浪瞬時壓力遽增
霎時四周水波雲氣變化萬千，$n" + HIW + "瞬間分不清劍在何方, 就在遲疑之
際，$N" + HIW + "一式" + HIC + "『冬陽天冰』" + HIW + "捲起狂霜亂雪直搗$n" + HIW + "心口。
    \n" + NOR,me,victim);
...
void sp_attack4(object me, object victim, object  weapon, int damage)
{
  int i;
  message_vision(HIR + "
$N" + HIR + "趁勢翻身飛躍，斜迴劍尖，念轉靈動使出" + HIW + "『紅染夕陽』" + HIR + "，霎時
...
void sp_attack5(object me, object victim, object  weapon, int damage)
{
  ...
  if(i>=sizeof(enemy)) i=0;
  if(!enemy[i]) i=0;
  if(undefinedp(enemy[i])) return ;
  
//出現率 1/2*1/7=~7%

  if( random(2) == 0 && me->query("force") > 510 && !me->query_temp("conti",1) && enemy[i]) {
    message_vision(HIR + "
眨眼間，劍鋒泛火，火光漸如燄日逼人，$N" + HIR + "如化祝融全身火紅。
驀地一聲大喝" + HIW + "『" + HIR + "烈" + HIW + "陽" + HIR + "十" + HIW + "七" + HIR + "式" + HIW + "』" + HIR + "，手中長劍猶似火龍摧天，劍幻千萬火
炬只在一瞬間$n" + HIR + "已完全火龍被吞沒
  ...
  ███  ████   ██████    ████     ██████      ████
   ...
   ██████    ███  ███▍  ███ ████▌   ██████◤
    ...
    if( random(me->query_cps()) < 3 ) {
      me->add("force",-100);
      message_vision("\n$N使完" + HIR + "『烈陽十七式』" + NOR + "後，由於定力不足，體內真氣一時無以為繼。\n",me);
      me->start_busy(1);
    }
  }
}
...
同時，長劍已離鞘直劃$n" + HIY + "的咽喉。
  ...
  if( weapon->query("id")=="sun_fire_sword" ||  weapon->query("id")=="sun_moon_sword") {
    message_vision(HIY + "
靈劍連陽隨著『朝陽再現』這一式散發出開天潛藏的靈氣，剎那間!劍泛金光
