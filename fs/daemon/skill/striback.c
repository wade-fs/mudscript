...
string *parry_msg = ({
 "$n瞧著$N的攻勢﹐腳步一轉﹐嚇然轉出了一式『邪嵐轉勢』﹐瞬間變制住了$N的攻勢﹐轉守為攻\n",
 "$n大喝一聲﹐趁$N擊出一擊重擊時﹐使出一招『兇兆舞勢落』﹐$N的身子硬是逆著$N的攻勢逼到$N身前\n",
 "$n注視著$N的層層攻勢﹐腳鋒一捷﹐點出了一記『殺元跺』﹐便趁著$N的攻勢尚未使盡前架住其鋒﹐硬是把$N元氣退回﹐使得$N元氣頗傷\n",
 "$n注意到$N即將展開攻勢﹐隨即將身法放輕﹐內力凝聚﹐用出了一招『厄驚覺』﹐$n看準了$N的攻勢尚未展開便扣住其身﹐虎口正對準了$N那尚未明瞭的臉孔上\n",
 "$n眼看著$N強大攻勢即將落在$n身上﹐忽然心神一震﹐腳步急轉成風﹐使出的正是『無心訣』﹐只見得$N的身子忽然凝滯不前﹐$n正在$N頭上攻下\n",
                    });
...
void striback(object me,object victim)
{
  ...
  {
    victim->start_busy(1);
    message_vision("$N成功\的將$n的攻勢反轉過去 !\n", me, victim);
  }
  else
  {
    message_vision("$N錯過了反擊良機!!\n", me, victim);
  }
}
