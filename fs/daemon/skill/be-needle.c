...
// 新增的招式對玩家和非玩家的威力有不同的計算
// 威力上參照了一些其他技能來加強，威力的增強
// 最主要的是參照一個function「夢靈轉」，因為
// 極為難蹲，所以威力蹲愈久的人會愈強，現在的
// 怪物以變態為主，織女實在難以生存，所以強度
// 上有很多考量了，希望非必要勿更動強度，如需
// 要更動，請在更動後知會一下我。
...
void sp1(object me, object victim, object weapon, int damage)
{
	...
	stabber = (int)(me->query_skill("stabber"));
	tim = (int)(me->query_function("magicpower")/10);
...
void sp4(object me, object victim, object weapon, int damage)
{
	...
	mam = (int)(me->query_skill("mamagic"));
	...
	if(me->query_temp("fire_tooth") == 1 || me->query_temp("ma-needle") == 1)
	{
	  ...
	  victim->receive_wound("kee", 99999);
	  ...
	  victim->receive_wound("kee", tim*(10*spi) );
...
void sp6(object me, object victim, object weapon, int damage)
{
	...
	me->add("mapower",random(3)+1);
        if(me->query_temp("ma-needle") == 1)
        {
          ...
          victim->receive_wound("kee", tim*fsk);
...
void sp7(object me, object victim, object weapon, int damage)
{
	...
	me->add("mapower",random(3)+1);
        ...
        victim->receive_damage("kee",tim*(10*spi));
        COMBAT_D->report_status(victim);
        }else{
...
void sp8(object me, object victim, object weapon, int damage)
{
	...
	me->add("mapower",random(3)+1);
        ...
        victim->receive_wound("kee", tim*(3*stabber));
        COMBAT_D->report_status(victim, 1);
        }else{
        message_vision(HIR + "$N暗暗散出數條無形氣勁向$n飛去，偷偷的射中了$n, 造成了數個不小的瘡口！\n" + NOR ,me,victim);
        victim->receive_damage("kee",tim*stabber);
        COMBAT_D->report_status(victim, 1);
        }
}
...
void sp9(object me, object victim, object weapon, int damage)
{
	...
	me->add("mapower",random(3)+1);
        ...
        victim->receive_damage("kee",tim*(3*stabber));
...
void sp11(object me, object victim, object weapon, int damage)
{
	...
	if( wizardp(me) )
	{
	message_vision("威力stabber現在「" + HIM+stabber+NOR + "」\n",me);
	}
...
void sp12(object me, object victim, object weapon, int damage)
{
	...
	magic = (int)(me->query_skill("magic"));
	stabber = (int)(me->query_skill("stabber"));
	exp = (int)(me->query("combat_exp")/10000);
	ena = (int)(me->query("atman_factor"));
	mam = (int)(me->query_skill("mamagic"));
	spi = (int)(me->query_spi());
	fsk = (int)(me->query_skill("force"));
	tim = (int)(me->query_function("magicpower")/10);
	ext = (int)(me->query("combat_exp")/1000000);

	me->add("mapower",random(5)+2);

	if( interactive(victim) )
	{
	pow=(int)(((mam+ena+exp)*tim)/80);
	}else{
	pow=(int)(((mam+ena+exp+stabber+spi+fsk+magic)*tim)/10);
	}

	if( wizardp(me) )
	{
	message_vision("威力pow現在「" + HIM+pow+NOR + "」\n",me);
	}
	...
	for(i=0; i<sizeof(enemy); i++)
	{
	  ...
	  for( j=0; j < tim; j++ )
	  {
	    ...
	    {
        enemy[i]->receive_wound("kee",pow*tim);
	    enemy[i]->receive_wound("gin",pow);
	    enemy[i]->receive_wound("sen",pow);
        COMBAT_D->report_status(enemy[i]);
	    }else{
        enemy[i]->receive_wound("kee",pow);
	    enemy[i]->receive_wound("gin",(int)(pow/2));
	    enemy[i]->receive_wound("sen",(int)(pow/2));
        COMBAT_D->report_status(enemy[i]);
	    }
	  }
...
void seven(object me, object victim,object weapon, int damage)
{
	...
	fsk = (int)(me->query_skill("force"));
	ext = (int)(me->query("combat_exp")/1000000);
	hit = (int)(this_player()->query_function("magicpower")/5);
	ext = (int)(this_player()->query("combat_exp")/1000000);
	tsk = (int)(victim->query("combat_exp")/500000);

	if( interactive(victim) )
	{
	dam = (int)(((mam+ena+exp)*hit)/(80+tsk));
	}else{
	dam = (int)(((mam+ena+exp+stabber+spi+fsk)*hit)/10);
	}

	if( wizardp(me) )
	{
	message_vision("基本威力dam現在「" + HIM+dam+NOR + "」\n",me);
	}
...
　　　　靈 － 光 － 隨 － 手 － 起" + HIY + "◎" + HIM + "靈 － 針 － 幻 － 七 － 式
	\n" + NOR),me,victim );
	victim->receive_wound("kee",(int)(dam*tsk));
	  for(i=0;i<=ext;i++)
	  {
	  switch(i)
	  {
	 ...
	 _<>_\\_\\<>/_/_<>_" + HBWHT+HIC + "☆靈亂乾坤幻無定☆" + NOR+HIC + "_<>_\\_\\<>/_/_<>_
	  <> / /<>\\ \\ <>                    <> / /<>\\ \\ <>
	    ...
	    enemy[j]->add("kee",-(int)(dam));
	    enemy[j]->add("force",-(int)(dam*tsk/500));
	    enemy[j]->add("gin",-(int)(dam*tsk/100));
	    enemy[j]->add("sen",-(int)(dam*tsk/100));
	...
	victim->receive_wound("kee",(int)(dam));
...
{
	int ski,mai,exi,mau;
	ski = (int)(this_player()->query_skill("be-needle",1));
	mai = (int)(this_player()->query_skill("mamagic",1));
	exi = (int)(this_player()->query("combat_exp",1)/500000);
	mau = (int)(this_player()->query_function("magicpower"));
...
{
        if(this_player()->query_skill("be-needle",1) >= 100)
        {
