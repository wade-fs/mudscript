//燎原百擊 by bss
// 調整為小mob使用的威力 by blazakira 2010/05/09
// 本技能只開放給npc使用
#include <ansi.h>
#include <combat.h>
...
{
	int ski,mai,exi,mau;
	ski = (int)(this_player()->query_skill("mob-lance",1)); //假設為100
	mau = (int)(this_player()->query_function("threeforce")); //假設為100 使用疊浪三重勁作為判斷之一
	...
	else if ( ski >= 100 && mau < 80 )
		return action [random(12)];
	else if ( ski >= 100 && mau <= 100  )
		return action [random(6)+6]; //即第七招開始使用 5+6=11=第十二招
	else
...
void sp1(object me, object victim, object  weapon, int damage)
{
   ...
   victim->receive_damage("kee",(victim->query("kee")/10),me); //秒殺的威力太強 故而改1/10
   victim->receive_wound("kee",(victim->query("kee")/10),me);
}
void sp2(object me, object victim, object  weapon, int damage)
{
 int skill_level=me->query_skill("mob-lance",1),bound;
 bound = victim->query("max_kee");
// bound = (bound/100)*5; //威力太高所以改弱
 bound = (bound/1000)*15 + (int)(this_player()->query("combat_exp",1)/50000); //假設為10000000/50000=200 即基礎傷害為玩家15/1000kee*10+exp加成
