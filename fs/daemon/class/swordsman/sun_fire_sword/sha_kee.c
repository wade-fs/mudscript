...
int perform(object me)
{
  ...
  return notify_fail("你無法使用連陽劍\n");
    if(!me) return 1;
    if(!enemy) return notify_fail("沒敵人..打屁阿..。\n");
      if(environment(me)!=environment(enemy))
...
if(environment(me)!=environment(target[j])) continue;
          ...
          me->add("kee",-((sha_value*2+kee_lv*2+sword_lv*2)/4));
          me->start_busy(1);
           me->add("force",-kee_lv*10);
}
      else if ( ski_value <  5 ){
           me->add("force",-kee_lv);
...
//roger 98/8/17
     ...
     for(j=0;j < i;j++){
          target[j]->receive_damage("kee",(sha_value*2+kee_lv*3+sword_lv*4));
// 沒有加 busy 沒關西但不能加上 no_power 吧
// 不然別派不就不用玩了
