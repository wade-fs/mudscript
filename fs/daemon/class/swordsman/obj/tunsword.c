...
{
   seteuid(getuid());
   set_name(HIB"迅"HIW"．"HIB"雷"HIW"．"HIB"劍"NOR,({"thunder_sword"}));
   set_weight(100);
   init_sword(115);
   if( clonep() )
...
int doo_wield(string str)
{
    ...
    message_vision("$N手中一閃,已將繞在腰間的"HIB"迅"HIW"．"HIB"雷"HIW"．"HIB"劍"NOR"威風凜凜的拿在手中。\n",me);
    tell_object(me,"迅雷劍法可和後三式併成迅雷十七式 !!\n而除了融入仙劍劍術外, 則亦可自成一套快絕的驚世劍法\n"); 
    me->set("get_sha_sp",0);
    return 0; 
         }   
}
...
int doo_unwield(string str)
{
    ...
    message_vision("$N一運內力,手中的"HIB"迅"HIW"．"HIB"雷"HIW"．"HIB"劍"NOR"剎時變成一把綿劍,$N順勢把它纏回腰間。\n",me);
    me->set("get_sha_sp",1);    
