// 2003/6 - 修正強度by Guetenr
//增加對 enemy[i]->die(); 的物件判斷 與 傷害函數enemy[i]->receive_wound(,,me) 補上攻擊方參數 by blazakira 2013/9/8
...
void hawk1(object me, object victim, object  weapon, int damage)
{
  ...
  if(70>random(100)) {victim->receive_damage("sen", random(victim->query("sen",1)/30)+victim->query("sen",1)/100, me); }
  COMBAT_D->report_status(victim, 1);
  if(70>random(100)) {victim->receive_damage("gin", random(victim->query("gin",1)/30)+victim->query("gin",1)/100, me); }
  COMBAT_D->report_status(victim, 1);
}
...
{
  int damage1;
  damage1  = victim->query("combat_exp",1)/15000;
  if(damage1 > 1300) damage1=1300;
  message_vision(sprintf(HIC"\n四周氣流瞬間如潮水動盪不停，$n"HIC"受潮流影響漸漸被捲入旋渦中動彈不得。\n"NOR),me,victim);
  victim->receive_damage("kee", damage1, me);
  COMBAT_D->report_status(victim, 1);
...
void hawk3(object me, object victim, object  weapon, int damage)
{
  ...
  victim->receive_damage("kee", random(victim->query("kee",1)/30)+victim->query("kee",1)/100, me);
  victim->add("force", -(victim->query("max_force",1)/200));
  COMBAT_D->report_status(victim, 1);
}
...
void hawk4(object me, object victim, object  weapon, int damage)
{
  ...
  if(hawkup > 2500) hawkup =2500;  //新增傷害最大值限制
...
void hawk5(object me, object victim, object  weapon, int damage)
{
  ...
  for(i=1;i<=6;i++)
  {
    ...
    if( 80 >random(100) )
    {
      ...
      victim->receive_wound("kee",400+random(300), me);  //修改連擊強度, 因為不會busy hawk本身
      if(i != 7)
        COMBAT_D->report_status(victim,1);
      else if( damage <=0 )
        COMBAT_D->report_status(victim,1);
...
void sp2(object me, object victim, object  weapon, int damage)
{
  ...
  victim->receive_damage("gin", 300, me);
  victim->receive_damage("sen", 300, me);
...
void invocation(object who,object room)
{
  ...
  message("vision",NOR+name() + HIY + "大聲的咆哮著:\n"HIR"                   愚蠢的人啊！竟敢傷害吾守護的天道門徒！！\n"
...
//以上部分區塊化, 因為上面的程式會使hawk和leader平分exp, 故略去 by Guetenr
    }
  }
  set_leader(who);
  command("defend "+who->query("id"));
  who->set_temp("have_hawk",1);
  if( who->query_temp("roared") )
  this_object()->set_temp("roared",1);
}
...
/*
  ...
  if( random(100) < 50 )
  {
    ...
    if( env == environment(target) )
    {
      ...
      if(kee > 2000) kee = 2000;  //新加心跳特攻之限制
      message_vision(HIY"四周捲起一陣氣流，將$N捲入天空重摔於地。\n"+NOR,target);
      target->receive_damage("kee", kee, me);
      target->receive_wound("kee", random(kee), me);
      target->receive_wound("sen", random(5), me);
      target->receive_wound("gin", random(5), me);
  ...
  if( ( me->query("kee") < me->query("max_kee")*0.1 ) && random(100) < 5 )
  {
    ...
    message_vision(HIG"靈鷹"HIC"眼見自己的生命如"NOR+RED"風中殘蠋"HIC"，仰天泣鳴不停，其聲"HIY"驚天地"HIC"，"HIG"泣鬼神"HIC"，瞬間"HIM"天地靈氣"HIC"齊聚，星象異變"
                   "\n\n                    "HIW"～　"HIR"鷹　　　魂　　　"HIY"泣　　　天　　　地　"HIW"～"NOR
                   "\n\n"HIM"靈鷹吸收日月精華，身影不斷的漲大，瞬息間奔向"NOR"$N"HIM"纏繞其身，悲鳴一聲，靈鷹自爆產生一股"
                   "\n巨大的毀滅力，令萬物毀滅，天地重生，周遭人物盡數毀滅，其主人受鷹魂所佑順利逃過此一劫數。\n"+NOR,enemy[i]);

    if( userp(enemy[i]) )
    {
      enemy[i]->receive_wound("gin", (enemy[i]->query("max_gin")/2)+random(enemy[i]->query("max_gin")/2), me);
      enemy[i]->receive_wound("kee", (enemy[i]->query("max_kee")/2)+random(enemy[i]->query("max_kee")/2), me);
      enemy[i]->receive_wound("sen", (enemy[i]->query("max_sen")/2)+random(enemy[i]->query("max_sen")/2), me);
      COMBAT_D->report_status(enemy[i],1);
    } else {
      pex = who->query("combat_exp");
      mex = enemy[i]->query("combat_exp");
      if( random(mex) < random(pex) )
      {
        if(enemy[i]) enemy[i]->unconcious(); //增加判斷句 by blazakira
        if(enemy[i]) enemy[i]->die(); //增加判斷句 by blazakira
      } else {
        enemy[i]->receive_wound("gin", (enemy[i]->query("gin")/4)+random(enemy[i]->query("max_gin")/4), me);
        enemy[i]->receive_wound("kee", (enemy[i]->query("kee")/4)+random(enemy[i]->query("max_kee")/4), me);
        enemy[i]->receive_wound("sen", (enemy[i]->query("sen")/4)+random(enemy[i]->query("max_sen")/4), me);
        COMBAT_D->report_status(enemy[i],1);
      }
    }
