...
// 為符合金刀門派特色作出名稱上的修正。 by frequency  2003/4/28
//增加環境的判斷以避免 【系統】0(0): player:0的錯誤 by blazakira 2011/2/28
//更改傷害判斷為receive_damage 但是另一個add("kee")的傷害根據敘述而不改變 by blazakira 2011/4/16
...
void create()
{
  ...
  {
    set("unit","把");
    set("long","一把由內勁聚集落葉所製成的刀。\n");
    set("value", 0 );
    set("material","crimsonsteel");
    set("wield_msg","$N聚氣凝神內勁一放，頓時柳葉聚集，手中多了一把刀身碧綠的$n。\n");
    set("unwield_msg","$N內勁一收，頓時落葉飛散，手中的$n也消失無蹤。\n");
    set("no_sell",1);
    set("no_give",1);
    set("no_save",1);
    set("no_get",1);
...
int do_hurt()
{
  object *enemy,wname,wna;
  int i,j,fun;
  if(!me || !environment(me)) return ;
  fun = me->query("functions/hurtkee/level");
  mkfun = me->query("functions/leaf-blade/level");
  wname=me->query_temp("secondary_weapon");
  wna=me->query_temp("weapon");
  if( !wna || ob->query("equipped") != "wielded" )
    return notify_fail("請把武器裝備上。\n");
  if( mkfun <= 10 )
    return notify_fail("你的聚葉成刀還沒到能使用這功\效的等級。\n");
  if( wna->query("id") == "mk-blade" )
    return notify_fail("此刀要裝備在副武器才有功\效。\n");
  if( me->query("force") < 600 )
    return notify_fail("你的內力不夠。\n");
