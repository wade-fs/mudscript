...
string ask_leader()
{
  ...
  string item_kinds;
  item_kinds="spells";
  if( me->query_temp("ask_leader/start")!=3 || me->query_temp("ask_leader/officer")==1 )
  {
    return 0;
  }
  if( !me->query("family") && !me->query("officer_class") ) {
    ...
    command("say 聖上派下來的任務，不分門派都要盡全力完成呀。");
  }
  ...
  me->set_temp("ask_leader/officer",1); //詢問過朝庭命官的紀錄mark
  return "";
}
