...
int perform(object me, object target)                                           
{
    ...
    if( hurt_value > 700) hurt_value=700;
        ...
        if( lv1 < 60  )
        return notify_fail("你的[30m玄混沌[0m練的不夠純熟﹐須要玄混沌 60 級。\n");      
        ...
        if(!(ob=me->query_temp("weapon")))                                        
        {
            ...
            return notify_fail("你不空手是無法使用[30m混沌地裂掌[0m的！！\n");
        }
        if(ob=me->query_temp("weapon"))                                         
        {                                                                       
        ...
        {                                                                       
            tell_object(me,"[30m混沌地裂掌[0m唯在戰鬥中才能使用。\n");
            return 1;
        }                                                                       
        message_vision("\t$N體內的真氣有如盤古開天地一般，真氣亂竄，\n\t似乎就要爆體而出, 只有將此一氣勁運輸到你的雙掌間中!!\n\t只見你雙手的筋肉暴漲了數倍之多!!\n",me,target);
        message_vision("$N雙拳漫佈混沌氣勁，只覺不吐不快，強招[30m混沌地裂掌[0m挾著轟然之勢向敵人而去 .[0m﹗\n",me,target);
                                                                                
        me->add("kee",-hurt_value/10);
        me->add("force",-hurt_value);
        if(80 > random(100))
        {                                                                       
        message_vision("$n的雙掌與[30m混沌地裂掌[0m相交較勁，卻慘遭雄渾的混沌內勁震斷雙臂，掌勁觸體後另你馬上開腸破肚！﹗\n",me,target);                                   
        ...
        {                                                                         
        message_vision("$n奮力向後一躍，閃避了[30m混沌地裂掌[0m的開山裂碑的殺傷力。\n",me,target); 
        message_vision( NOR,me);                                        
        target->receive_damage("kee",hurt_value/14);
        target->add("force",-hurt_value/18);
        COMBAT_D->report_status(target);                                
        }                                                                        
        message_vision("[34m「混沌地裂掌」[0m使出後，$N立刻縱身撲進$n，繼續搶攻",me,target); 
        me->start_busy(1);
