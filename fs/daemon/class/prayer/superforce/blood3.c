...
int perform(object me, object target)                                           
{
 ...
 if((string)ob->query("skill_type") != "unarmed")
        ...
        {                                                                       
            tell_object(me,"穹蒼無悔唯在戰鬥中才能使用。\n");
            return 1;
        }                                                                       

        message_vision("\t$N蔚藍的天空開始變成血紅色, 方圓十里之內如同掉入阿修羅地獄般!!\n\t$N近似瘋狂地唸著渾天寶鑑的練功\口訣, 滿天的血紅氣勁越來越濃郁!!\n\t血龍捲風應勁而生了!!\n\n",me,target);
        message_vision("$N吼道:是你逼我使用 穹蒼無悔的!!去見閻王吧!!\n\n",me,target);
        me->add("kee",-(hurt_value/20));
        me->add("force",-(hurt_value/4));
        if(70>random(100))
        {                                                                       
        message_vision("$n避無可避﹐霎時被高速而來的血龍捲風捲成一團﹐被旋風拉扯的痛楚有如撕肝裂肺﹗\n",me,target);                                                                   
  ...
  target->receive_damage("kee",hurt_value);
        ...
        {                                                                         
        message_vision("$n運功\抵禦﹐雖然依舊被血龍捲風捲身而入﹐可是$n的內功\修為不俗, 所以仍然是撿回一條小命!!。\n",me,target);  
        target->receive_damage("kee",hurt_value/10);
        target->add("force",-hurt_value/10);
        COMBAT_D->report_status(target);                                
        }                                                                        
        message_vision("「穹蒼無悔」使出後，$N大耗內力、天闕穴驟然疼痛，頹然跪倒在地\n",me,target); 
   me->start_busy(1);
