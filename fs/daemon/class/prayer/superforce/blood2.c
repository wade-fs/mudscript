...
int perform(object me, object target)                                           
{
  ...
  if( hurt_value > 2000) hurt_value=2000;
        ...
        if( lv1 < 70 )
           return notify_fail("你的血穹蒼練的不夠純熟﹐無法催運出穹蒼血球，須70級。\n");      
        ...
        if(!(ob=me->query_temp("weapon")))                                        
        {
            ...
            return   notify_fail("不空手怎麼將穹蒼血球聚於手上?\n");
        }
        if(ob=me->query_temp("weapon"))                                         
        {                                                                       
 if((string)ob->query("skill_type") != "unarmed")
        ...
        {                                                                       
            tell_object(me,"穹蒼血球唯在戰鬥中才能使用。\n");
            return 1;
        }                                                                       
        message_vision("\t\t$N體外血紅的氣勁隨意而走,\n\t\t只見$N身旁的血紅色蒸氣越來越濃厚!!\n\t\t$N大喝一聲,努力將氣勁聚於手掌之上, 形成了一團\n\t\t直徑寸許\的血球!!!\n\n",me,target);
        message_vision("$N運勁於雙掌，使勁一推,穹蒼血球朝敵人狂奔而去!\n",me,target);                 
                                                                                
        me->add("kee",-(hurt_value/20));
        me->add("force",-(hurt_value/4));
        if(70>random(100))
        {                                                                       
        message_vision("$n一見躲避已經無望，運起護身氣勁硬接下這團血球，無奈血穹蒼氣勁太猛，護身氣勁慘遭震碎，霎時高熱的血燄焚燒$n的全身﹐血液好似被蒸發而出!\n",me,target);
        ...
        {                                                                         
        message_vision("$n以全身的內力組成氣牆，與血穹蒼氣勁互鬥，[砰]的一聲，被震退數步，口吐鮮血。\n",me,target);                                                                            
        message_vision( NOR,me);                                        
        target->receive_damage("kee",hurt_value/10);
        target->add("force",-hurt_value/10);
        COMBAT_D->report_status(target);                                
        }                                                                        
        message_vision("「穹蒼血球」使出後，$N內力大量耗損、心坎穴驟然劇痛，$N黯然跌坐在地\n",me,target); 
     me->start_busy(1);
