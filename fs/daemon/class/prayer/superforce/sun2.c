...
int perform(object me, object target)                                           
{
    ...
    if( hurt_value > 900) hurt_value=900;
        ...
        if( lv1 < 60  )
            return notify_fail("你的金晨曦練的不夠純熟﹐無法催運出金晨曦的霸氣，須60級。\n");
        ...
        if(!(ob=me->query_temp("weapon")))
        {
            ...
            return notify_fail("你不空手是無法使用雙星趕月的！！\n");
        }
        if(ob=me->query_temp("weapon"))                                         
        {                                                                       
        ...
        {                                                                       
            tell_object(me,"雙星趕月唯在戰鬥中才能使用。\n");
            return 1;
        }                                                                       
        message_vision("\t$N全身金光閃閃，光華奪目，金色的真氣在身旁旋繞，\n\t$N整個人飛昇在數丈高的半空中!!光芒四射宛若驕陽!!\n\n",me,target);
        message_vision("$N雙拳一握，兩團耀眼金光匯聚成球，呼嘯而出！此招有如雙龍出海，金光刺目，直逼敵人上下兩路﹗\n",me,target);
        ...
        {                                                                         
        message_vision("$n竭力閃避，避開了雙星趕月正面的攻擊，兩團金光轟向地面將地面炸了開來。\n",me,target);
        message_vision( NOR,me);                                        
        target->receive_damage("kee",hurt_value/12);
        target->add("force",-hurt_value/14);
        COMBAT_D->report_status(target);                                
        }                                                                        
        message_vision("「雙星趕月」使出後，$N面露笑容，似乎很滿意此招的攻擊能力",me,target); 
        me->start_busy(1);
