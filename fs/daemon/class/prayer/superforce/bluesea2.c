...
skill=this_player()->query("functions/bluesea/level");
        ...
        if(skill < 60  )
            return notify_fail("你的[34m靛滄海[0m練的不夠純熟﹐無法催運出[34m繭困勢[0m，須要60級。\n");      
        ...
        if(!(ob=me->query_temp("weapon")))
        {
            ...
            return notify_fail("你不空手是無法使用[34m繭困勢[0m的！！\n");
        }
        if(ob=me->query_temp("weapon"))                                         
        {                                                                       
        ...
        {                                                                       
        tell_object(me,"[34m繭困勢[0m唯在戰鬥中才能使用。\n");              
        return 1;
        }                                                                       
        if(target->is_busy())
        return notify_fail("敵人 busy 中，快攻擊吧。\n");
        message_vision("$N周遭澎湃的氣勁如潮水般地快速奔走，$N將此一氣勁越旋越快!!\n\n",me,target);
        message_vision("忽然$N內勁一送﹐整個[34m繭困勢[0m的氣勁越旋越緊地向敵人席捲而下 ﹗\n",me,target);
                                                                                
        me->add("force",-skill*3);

if(80>random(100))
        {                                                                       
        message_vision("$n被你的[34m繭困勢[0m內勁給緊緊的困住，無法動彈！﹗\n",me,target); 
        ...
        {                                                                         
        message_vision("$n護身氣勁受到感應，股地將全身的內勁爆發出來，化解了[34m繭困勢[0m窒礙的氣流 。\n",me,target);                                                                            
        message_vision( NOR,me);                                        
        COMBAT_D->report_status(target);                                
        }
        message_vision("[34m「繭困勢」[0m使出後，$N急忙呼息調氣，重組戰鬥力",me,target); 
        me->start_busy(1);
