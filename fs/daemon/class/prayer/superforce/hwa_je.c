...
if(me->query("class")!="prayer")
  ...
  if (me->query_temp("supertengin/wind") || me->query_temp("supertengin/water") ||
      me->query_temp("supertengin/mountain") || me->query_temp("supertengin/fire") ||
      me->query_temp("supertengin/thunder"))
     return notify_fail("你正在使用寰宇渾天勁，沒有多餘的力量推動化劫。\n");

  if ((int)me->query_temp ("hwa_je", 1))
     return notify_fail ("你已經使用化劫了。\n");
