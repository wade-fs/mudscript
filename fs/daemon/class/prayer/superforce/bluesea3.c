...
skill=this_player()->query("functions/bluesea/level");
        ...
        if(skill < 90  )
            return notify_fail("你的[34m靛滄海[0m練的不夠純熟﹐無法催運出[34m鯨吞勢[0m，須90級。\n");      
        ...
        if(!(ob=me->query_temp("weapon")))
        {
            ...
            return notify_fail("你不空手是無法使用[34m鯨吞勢[0m的！！\n");
        }
        if(ob=me->query_temp("weapon"))                                         
        {                                                                       
...
if(80>random(100))
        ...
        {                                                                         
                message_vision("$n急中生智，將[34m鯨吞勢[0m的內勁集中在手上後，盡瀉於地 。反而使得$N失去了著力點，喪失了平衡感!!\n",me,target);
                COMBAT_D->report_status(target);                                
        }                                                                        
        message_vision("[34m「鯨吞勢」[0m使出後，$N急忙呼息調氣，重組戰鬥力\n",me,target); 
        me->start_busy(1);
