...
int perform(object me, object target)                                           
{
    ...
    if( hurt_value > 800) hurt_value=800;
        ...
        if( lv1 < 30  )
            return notify_fail("你的金晨曦練的不夠純熟﹐無法催運出金晨曦的霸氣，須30級。\n");      
        ...
        if(!(ob=me->query_temp("weapon")))                                        
        {
            ...
            return notify_fail("你不空手是無法使用金劍穿雲的！！\n");
        }
        if(ob=me->query_temp("weapon"))                                         
        {                                                                       
        ...
        {                                                                       
            tell_object(me,"金劍穿雲唯在戰鬥中才能使用。\n");
            return 1;
        }                                                                       
        message_vision("\t$N全身金光閃閃，光華奪目，金色的真氣在身旁旋繞，\n\t$N整個人飛昇在數丈高的半空中!!光芒四射宛若驕陽!! \n\n",me,target);
        message_vision("$N單手一指，一道耀眼金光激射而出，正是霸招金劍穿雲！此招有如飛龍撲日，豪光萬千﹗\n",me,target);                 
                                                                                
        me->add("kee",-(hurt_value/20));
        me->add("force",-(hurt_value/4));
        if(80>random(100))
        {                                                                       
        message_vision("$n竭力抵擋，仍被霸道的金晨曦內勁射中，頓時金光爆體，向四方亂射！﹗\n",me,target); 
        ...
        {                                                                         
        message_vision("$n竭力抵擋，雖避開了金劍穿雲正面的攻擊，不過右手卻還是被射穿了。\n",me,target); 
        message_vision( NOR,me);                                        
                target->receive_damage("kee",hurt_value/13);
                target->add("force",-hurt_value/16);
        COMBAT_D->report_status(target);                                
        }                                                                        
        message_vision("「金劍穿雲」使出後，$N面露笑容，在半空中俾睨著$n",me,target); 
        me->start_busy(1);
