...
skill=me->query("functions/bluesea/level");
        ...
        if(skill < 30  )
            return notify_fail("你的靛滄海練的不夠純熟﹐無法催運出滔天勢，須要30級。\n");      
        ...
        if(!(ob=me->query_temp("weapon")))
        {
            ...
            return notify_fail("你不空手是無法使用滔天勢的！！\n");
        }
        if(ob=me->query_temp("weapon"))                                         
        {                                                                       
        ...
        message_vision("$N將這股氣勁射出﹐有如浪潮滔天的氣勁往敵人下方洶湧衝突而上 ﹗\n",me,target);                 
                                                                                
        me->add("force",-skill*2);

if(80>random(100))
        {
                message_vision("$n被你滔天勢的內勁給拋上了天空，整個人失去了重心！﹗\n",me,target);                                                                   
        ...
        {
                message_vision("$n忙使出千斤墜，將身形穩住，化解了滔天勢拋捲的氣勁 .。\n",me,target);                                                                            
                message_vision( NOR,me);                                        
                COMBAT_D->report_status(target);                                
        }
        message_vision("「滔天勢」使出後，$N急忙呼息調氣，重組戰鬥力",me,target); 
        me->start_busy(1);
