...
int do_nod()
{ //因為nod是emote，所以如果用notify_fail()去回應的話
  //因為，這object在房間物件中的一個
  //所以，遇到noitfy_fail，會去搜尋在這裡符合的COMMAND
  //因為，會符合房間的emote動作，所以notify會忽略目前的nod
  //而去當成是房間的emote，因此變成此段成是無效化
...
ob = new("/autoload/killer/killer_card.c");
ob->move(this_player());
