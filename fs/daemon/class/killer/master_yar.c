...
//2001.11.5 為了殺手進階職業，加入幻之忍殺的任務 by wataru
...
string to_hurt()
{
  ...
  tell_object(me,"或許\你將他們的定情信物-滅神匕首。交給靜柔她，也許\她會想起當時的甜蜜\n");
...
string retitle()
{
  ...
  if (me->query("class") != "killer")
   return "媽的。。離開殺手，還想要黑牙連的地位？？";

/*
  if (me->query("title"))
  { //當官回來會是黑牙連第三代弟子
    if (me->query("title") != "黑牙聯第三代弟子")
      return "神經病，你的稱號不是好好的？";
  }
*/

  else if (me->query("quest/head-kill") == 1)
  {
    tell_object(me,"稱號怎麼不見了呢？該小心一點。\n");
    tell_object(me,"就讓為師的來回復你的稱號吧！\n");
    me->set("title",HIR + "當"+HIG + "代"+HIY + "殺"+HIB + "手" + NOR);
    return "好好的加油吧！！！";
  }
  else if (me->query("quest/memory") == 1)
  {
    tell_object(me,"稱號怎麼不見了呢？該小心一點。\n");
    tell_object(me,"就讓為師的來回復你的稱號吧！\n");
...
string to_miss()
{
  ...
  tell_object(me,"目前有(忍者),(黑血神針),（追殺）,(人頭),(幻之忍殺)五個任務\n");
  return "你想出任務嗎？";
}
...
string to_miss4()
{
  ...
  if(present("black blood",me))
  {
    ...
    tell_object(me,"目前，那位拿著黑血神針到處傷人的人，似乎已經被黃龍祖組長發現了\n");
    me->set_temp("mission3",1);
    return "$N可以去找他問看看(追殺者)";
  }
  return "$N你到底知不知道為何要追殺呀！帶著黑血神針再來找我！";
}
...
string to_miss6()
{
  ...
  if (!me->query("quest/head-kill"))
    return "連殺手全力追殺的人頭都拿不到，你又如何有辦法對付東瀛忍者呢？";
  if (!me->query("quest/island"))
    return "幻之忍殺？你不懂啦.....連個東南西北都搞不清楚....";
  if (me->query("combat_exp") < 2000000)
    return "雖然你已獲得當代殺手的稱號，但是實力還是不太夠";

  if (!me->query_temp("firstpart"))
  {
    tell_object(me,"我聽魯仝提到，上次你們前去尋找東瀛島........\n");
    tell_object(me,"相信你也知道東瀛忍者的勢力最近似乎在國內發展起來\n");
    tell_object(me,"還有當年我的師父葉孤城前往東瀛,這幾年來也絲毫沒有音信\n");
    tell_object(me,"還有楊小邪也在東瀛那邊分析東瀛忍者的派流............\n");
    tell_object(me,"但當年的情況所實在接觸太少了,就連東瀛島所在，我也一無所知...\n");
    tell_object(me,"你就去找一下魯仝的兒子，看看是否知道(東瀛島)所在？\n");
...
int accept_object( object me , object ob )
{
  ...
  message("system",HIM + "葉秀殺大笑：『哈哈。。。江山代有才人出！一代新人換舊人\n\n"+me->query("name")
...
//  if( me->query("title") != HIR + "殺手全力追殺的人頭" + NOR )
  if (!me->query("killyar"))
    return "沒事接受什麼血的試煉？?";
  message_vision("$N夠勇氣，閉上眼睛，我送你去吧！。\n",me);
  me->move("/open/killer/headkill/killtest/flowrm1.c");
  me->save();
  return "破陣口訣：逢三減一，十丈反左";
}
...
手中。\n"+
  ...
  message("system",HIR + "可惡！！紅葉組，銀狼組！把"+winner->query("name")+"人頭提來
見我！\n" + NOR,users());
  ...
  guard=all_inventory(this_object());
  for( j=0;j<sizeof(guard);j++ ) {
    if( !guard[j]->query("guard_yar") ) continue;
    guard[j]->command("defend master yar");
  }
  :: unconcious();
}
...
void die()
{
  ...
  if(winner)
  {
    obj = new ("/open/newhand/obj/blackstone.c");
    obj->set("kill_id",winner->query("id"));
    obj->move(winner);
  }
  if (winner->query("class") != "killer")
  {
    ...
    if(userp(winner))
    {
      if(winner->query("combat_exp") > 450000)
      {
        ...
        winner->set("title",HBRED + "靠別人殺葉秀殺的爛人" + NOR);
    }
    else
      winner->set("title",HIG + "大白癡" + NOR);
  }
  if(userp(winner) && winner->query_temp("not_robot") > time() )
  {
    if ( winner->query_temp("bless")==1 )
    {
      ...
      if( j==7 || j==77 || j== 777 || j==1111 || j==55 || j==555 || j==1000 || j==4000 || j==3333 || j==2222 )
      {
        ...
        message_vision(HIM + "\n從葉秀殺的身上掉下了一件奇怪的東西!!\n" + NOR,winner);
        write_file("/log/sky/obj4/will_shells",sprintf("%s(%s) 讓葉秀殺掉下了意念之貝殼於 %s\n",
