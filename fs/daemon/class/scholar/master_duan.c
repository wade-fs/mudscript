//為了增加新手quest於門派首領身上 因而取消雙色字 以方便寫作XD by blazakira 2011/5/5
...
void create()
{
  ...
  set("max_gin",10000);
  set("max_sen",10000);
//  set("kee",10000); //使用系統預設的外加素質 反而比較高 by blazakira
//  set("gin",10000);
//  set("sen",10000);
  set("str", 44);
  set("cor", 24);
  ...
  if( random(4) < 1 ){
    if( is_fighting() ){
      if( query("kee") < query("eff_kee") )
        command("exert recover"); //戰鬥中回氣反而會busy自己的說 不過原設計如此就不更動了 by blazakira
    }
    else{
...
string do_linpo()
{
  ...
  if(me->query("family/family_name")!="段家")
    return "你問太多了吧，這不關你的事!!";
  if(me->query_skill("linpo-steps",1)!=100){
    command("slap "+me->query("id"));
    return "你基本的都還沒學好，就在想什麼秘笈???";
  }

  if(!me->query("quests/catch_thief"))
    tell_object(me,"段雲小聲的告訴你:唉，家醜不可外揚，你還是別再問了!!\n");
  else
    tell_object(me,"段雲生氣的說:哼，還不是被趙琴風那個叛徒給偷走了!!\n");
...
int accept_object(object me, object obj)
{
  ...
  else if(obj->query("name")=="凌波微步捲軸" && obj->query("linpo_quest"))
  {
    if(me->query("marks/ask_linpo")!=3){
      ...
      command("say 沒錯，就是這本秘笈");
      command("think");
      command("hmm");
      command("say 如果我沒記錯的話，這東西少了幾頁，前面所記載的這些我都已經教過你了!!\n");
      command("give "+obj->query("id")+" to "+me->query("id"));
      me->set("marks/ask_linpo",4);
      command("say 如果你真的想把凌波微步的精華學會，就去找段譽吧!!\n");
    }
  }
  else
  {
  ...
  if(userp(winner) && winner->query_temp("not_robot") > time() )
  {
    if ( winner->query_temp("bless")==1 )
    {
      ...
      if( j==7 || j==77 || j== 777 || j==1111 || j==55 || j==555 || j==1000 || j==4000 || j==3333 || j==2222 )
      {
        ...
        message_vision(HIM + "\n從段雲的身上掉下了一件奇怪的東西!!\n" + NOR,winner);
        write_file("/log/sky/obj2/peace_stone",sprintf("%s(%s) 讓段雲掉下了和平石於 %s\n",
