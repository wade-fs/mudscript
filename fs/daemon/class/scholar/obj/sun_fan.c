...
void create()
{
     set_name("烈陽扇",({"sun fan"}) );
     set_weight(10000);
     if( clonep() ) 
               set_default_object(__FILE__);
     else {
     set("long","採用火燄山裡的礦石造的鐵扇,戰鬥時會招來高熱.\n");
            set("unit", "件");             
            set("value",10000);
            set("material","steal");
           }         
...
{ 
     ...
     if( query("equipped") ) 
     {
        ...
        message_vision("烈陽扇感受$N的恨意,亮起了一道火紅色的光.\n",user);
        set_heart_beat(1);
     } 
}
...
{  
   if(str=="ruby_ring" || str=="all")
     if( query("equipped") )
     {
   message_vision("$N卸下烈陽扇,烈陽扇的烈火勁瞬時消失.\n",user);
        set_heart_beat(0);
     }    
}    
 ...
 if( user->is_fighting() && query("equipped") )
  {
    if( sp > random(260) )
    {
     ...
     {
message_vision(HIB+"烈陽扇的烈火勁滔滔不絕向$N狂奔而去!!!\n"+NOR,enemy[i]);
      enemy[i]->receive_damage("kee",pp,user);
      COMBAT_D->report_status(enemy[i], 0);
     }else
message_vision(HIB+"烈陽扇的烈火勁滔滔不絕向$N狂奔而去!...但是沒中...\n"
                     +NOR,enemy[i]);         
     }
  }
  return;
