...
void create()
{
     set_name("蒼森扇",({"torch fan"}) );
     set_weight(10000);
     if( clonep() ) 
               set_default_object(__FILE__);
     else {
     set("long","利用深山中的神木所造的木扇,戰鬥時引來藤蔓襲人.\n");
            set("unit", "件");             
            set("value",10000);
            set("material","wood");
           }         
...
{ 
     ...
     if( query("equipped") ) 
     {
        ...
        message_vision("蒼森扇感受$N的怨氣,亮起了一道草綠色的光.\n",user);
        set_heart_beat(1);
     } 
}
...
{  
   if(str=="ruby_ring" || str=="all")
     if( query("equipped") )
     {
   message_vision("$N卸下蒼森扇,蒼森扇招來的藤蔓瞬時遁入地裡.\n",user);
        set_heart_beat(0);
     }    
}    
 ...
 if( user->is_fighting() && query("equipped") )
  {
    if( sp > random(260) )
    {
     ...
     {
message_vision(HIB+"蒼森扇招來的藤蔓穿$N身體而過!!!\n"+NOR,enemy[i]);
      enemy[i]->receive_damage("kee",pp,user);
      COMBAT_D->report_status(enemy[i], 0);
     }else
message_vision(HIB+"蒼森扇招來的藤蔓穿$N身體而過!...但是沒中...\n"
                     +NOR,enemy[i]);         
     }
  }
  return;
