...
//一般ppl大約只會到2100 + weapon 只能到2200
      ...
      message_vision( HIR + @LONG
$N熟知兵法運計之要，將儒門火計以及六韜奇略中大熱之計，和而為一$n驟時發現身旁業火亂竄，竟然是赤壁之戰時所用之『火    龍    計』!
LONG + NOR , me , all[i] );
      all[i]->apply_condition("burn",30);  
     all[i]->receive_wound( "kee" ,dam1*10);
    ...
    {
dam=all[i]->query("max_kee");
       message_vision( HIR + @LONG
$n來不及逃跑﹐因此被$N放出了火燄燒傷了﹗
LONG + NOR , me , all[i] );
       all[i]->apply_condition("burn",10);  
     all[i]->receive_wound( "kee" ,dam*10);
       COMBAT_D->report_status( all[i] );
     }
  }
  else 
...
if (me->query("quest/gold-fire") && me->query("env/大火計"))
 {
  call_out("again",2,me);
}
  return 1;
...
//加入temp
    message_vision( HIR + @LONG
$N大喊  : 『六韜奇略，烈火焚天 - 炬火計』一個巨大的火陣在眾人面前展開!!
LONG + NOR , me );
    for(i=0;i<sizeof(all);i++)
    {
        if(random(100) <80)
