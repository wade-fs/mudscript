...
{
	object ob;
//限制npc不準帶diamond以上的錢 by Neverend
	string type;
	if (sscanf(file,"/obj/money/%s",type) == 1)
	{
	  if( type == "diamond" ) return 0;
	}
...
{
	object ob;
//限制npc身上能帶的錢最大值 by Neverend
	ob = carry_object("/obj/money/" + type);
	if( !ob ) return 0;
	if( (type == "cash") && (amount > 20) )		amount=20;
	if( (type == "gold") && (amount > 2000) )	amount=2000;
...
int random_move()
{
	...
	if( !query("random_move") )
	set("random_move",query("chat_chance"));

    if (query("random_move") <= 1)
	{
	add("random_move",100);
