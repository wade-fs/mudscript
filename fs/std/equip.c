...
int query_autoload()
{
//add,wiz 的裝備都可以save by bss
  if(environment()){
    if(wizardp(environment())) return 1; // set("autoload",1);
  }
//  return query("autoload") && query("equipped");
	return query("equipped") && ! query("no_save");
}
void setheartbeat(int i) { set_heart_beat(i); }
