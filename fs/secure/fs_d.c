...
string init_fsgoto(object me, string mudlib_id) {
    ...
    if (mudlib_id == load_object("/secure/system_d")->query_mudlib_id()) return "你已經在本機伺服器了。\n";
...
void p2p_send_fs_presence(string to_mudlib, string action,
    ...
    string msg = "fs_presence|" + load_object("/secure/system_d")->query_mudlib_id() + "|" + to_mudlib + "|" +
...
void p2p_send_fs_query(string mudlib_id, string type, string payload) {
    p2p_broadcast("fs_query|" + load_object("/secure/system_d")->query_mudlib_id() + "|" + mudlib_id + "|" + type + "|" + payload);
}

void handle_fs_query(string from_mudlib, string type, string payload) {
    if (type == "info") {
        string resp = load_object("/secure/system_d")->query_mudlib_id() + "|info|" + FS_MUDLIB_NAME + "|" + START_ROOM;
        p2p_broadcast("fs_resp|" + load_object("/secure/system_d")->query_mudlib_id() + "|" + from_mudlib + "|info|" + resp);
    } else if (type == "list") {
        string resp = FS_MUDLIB_NAME + "|" + load_object("/secure/system_d")->query_mudlib_id();
        mixed ks = keys(joined_muds);
        foreach (string mid in ks) {
            resp += "," + joined_muds[mid]["name"] + "|" + mid;
        }
        p2p_broadcast("fs_resp|" + load_object("/secure/system_d")->query_mudlib_id() + "|" + from_mudlib + "|list|" + resp);
    } else if (type == "room") {
        string src = read_file("/mudlib" + payload);
        if (!src) src = read_file(payload);
        if (!src) src = "";
        p2p_broadcast("fs_resp|" + load_object("/secure/system_d")->query_mudlib_id() + "|" + from_mudlib + "|room|" + payload + "|" + src);
    }
}
