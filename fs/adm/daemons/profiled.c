...
void make_profile(string sort_by)
{
	...
	for(i=0; i<sizeof(entry); i++)
		msg += sprintf("%-15s%-15d%-15d%-15d%-15d\n", entry[i],
			...
			command_log[entry[i]]["mem"]/command_log[entry[i]]["count"],
			command_log[entry[i]]["stime"]*100/command_log[entry[i]]["count"],
			command_log[entry[i]]["utime"]*100/command_log[entry[i]]["count"]);
	rm("/PROFILE");
