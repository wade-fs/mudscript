...
([
        "sweet":([
                "action":               "$N用牠的尾巴掃向$n的$l",
                "damage":               20,
                "damage_type": "瘀傷",
                ]),
        "dunt": ([
                "action":               "$N用頭上的角撞向$n的$l",
                "damage":               100,
                "damage_type": "刺傷",
                ]),
        "hoof": ([
...
void setup_beast(object ob)
{
        ...
        if( undefinedp(my["max_gin"]) ) {
                if( my["age"] <= 3 ) my["max_gin"] = 500;
                else if( my["age"] <= 10 ) my["max_gin"] = 500 + (my["age"] - 3) * 20;
                else if( my["age"] <= 30 ) my["max_gin"] = 640 + (my["age"] - 10) * 5;
                else my["max_gin"] = my["max_gin"] = 640 + (my["age"] - 30);
        }
        if( undefinedp(my["max_kee"]) ) {
                if( my["age"] <= 5 ) my["max_kee"] = 500;
                else if( my["age"] <= 20 ) my["max_kee"] = 500 + (my["age"] - 5) * 25;
                else my["max_kee"] = my["max_kee"] = 625 + (my["age"] - 20) * 5;
        }
        if( undefinedp(my["max_sen"]) ) {
                if( my["age"] <= 20 ) my["max_sen"] = 500;
                else my["max_sen"] = 500 + (my["age"] - 20) * 10;
        }
