...
// 加在底下的是允許有寫的權力, 各區域負責人應該有讀寫該區域的能力
...
private mapping exclude_read = ([
        ...
        "feature"     :       ({ "(immortal)","(apprentice)","(wizard)" }),
        // immortal 只要專心讀文件檔即可
        "doc"         :       ({ }),
// 由於以下的限制造成 adm 以下的 wiz 都不能 wear eq 所以改成
// 只限制 immortal by swy
        "std" : ({"(immortal)"}),
...
//        "u"           :       ({ "(immortal)","(apprentice)","(wizard)","(guest)" }),
        ...
        "daemon"      :       ({ "(immortal)","(apprentice)", "(guest)" }),
...
int valid_write(string file, mixed user, string func)
{
        ...
        if( !objectp(user) )
                error("SECURITYD->valid_write: user 的參數錯誤.\n");
...
/*
        if( file == "/adm/daemons/securityd.c" )
          return 0;
*/
        // 🚀 關鍵修正：使用 Go-style 判定路徑
        dir = base_name(user);
        if( str_has_prefix(dir, "/area/lm/") && str_has_prefix(file, "/data/lm") )
                return 1;
...
*/
        ...
        // Get the euid and status of the user.
        euid = geteuid(user);
        if( !euid ) return 0; // 沒 euid 怎麼可以讀寫咧         Arthur
