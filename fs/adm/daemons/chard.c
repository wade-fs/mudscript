...
{
	int i,exp;
	object corpse,ob, *inv, *obj=({});

	if( victim->is_ghost() ) return 0;
  if(killer->query_temp("spirit_curse"))
  {
    killer->apply_condition("spirit_curse",
        killer->query_condition("spirit_curse") + 1 );
  }
	corpse = new(CORPSE_OB);
	corpse->set_name( victim->name(1) + "的屍體", ({ "corpse" }) );
	corpse->set("base_file",base_name(victim));
  ...
  corpse->set("kill_id",killer->query("id"));
	corpse->set_weight( victim->query_weight() );
	corpse->set_max_encumbrance( victim->query_max_encumbrance() +100 );
	...
	// 讓玩家從屍體中不一定拿得到eq and weapon
	// 相關設定只要在eq and weapon 中設定 get_chance
	// 就可以了...
	//                  modified by Konn
	//
		while( i-- ) {
/*
	...
	case 10001..100000:
	  if( random(50) == 5 )
	  {
	  ob=new("/obj/stone/powder")->move(corpse);
	  }
	break;
	case 100001..1000000:
	  if( random(50) == 5 )
	  ...
	  ob=new("/obj/stone/powder")->move(corpse);
	    ...
	    if( random(500) == 5 )
	    {
	    ob=new("/obj/stone/suipian")->move(corpse);
	    }
	  }
	break;
	case 1000001..10000000:
	  ...
	  ob=new("/obj/stone/powder")->move(corpse);
	    ...
	    ob=new("/obj/stone/suipian")->move(corpse);
	      ...
	      if( random(-1) == 5 )
	      {
	      ob=new("/obj/stone/jiao")->move(corpse);
	      }
	    }
	  }
	break;
	  ...
	  ob=new("/obj/stone/powder")->move(corpse);
	    ...
	    ob=new("/obj/stone/suipian")->move(corpse);
	      ...
	      ob=new("/obj/stone/jiao")->move(corpse);
	        ...
	        if( random(50000) == 5 )
		{
		ob=new("/obj/stone/stone")->move(corpse);
		}
	      }
	    }
	  }
