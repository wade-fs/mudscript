...
void create()
{
        ...
        seteuid(getuid());
        set_name("新聞管理器",({"msgd","msg"}));
        set("channel_id","訊息通告");
        if (file_size(MSGFILE+".o")<=0){
                setup();
                msg=({});
                add("msg",msg);
                save();
        ...
        if (clonep())
                return;
        CHANNEL_D->do_channel( this_object(), "sys", "新聞系統已經啟動。\n");   
        call_out("on_time",10);
}
