...
object connect(mixed arg)
{
        ...
        case HTTPD_PORT:
                return new("/obj/http");
	      case 5556:
                err = catch(login_ob = new("/adm/obj/ftplogin"));
        ...
        case 4002:
        case 4003:
                err = catch(login_ob = new("/adm/obj/channelobj"));
                    return login_ob;
            break;    
        default:
...
static void crash(string error, object command_giver, object current_object)
{
        efun::shout("系統核心發出一聲慘叫﹕哇─哩─咧─\n");
        efun::shout("系統核心告訴你﹕要當機了﹐自己保重吧﹗\n");
        log_file("static/NEW_CRASHES", MUD_NAME + "當機於: " + ctime(time()) +
                ", 原因: " + error + "\n");
        if (command_giver) {
...
string standard_trace(mapping error, int caught)
{
    ...
    if(strsrch(error["error"],"evaluation")!=-1){
      if(this_player()){
        ...
        tell_object(this_player(),"發生計算溢位錯誤, 請將畫面, 包含之前所做的cmds post to wiz, bow!!\n");
      }
    }
    CHANNEL_D->do_channel(this_object(),"sys",sprintf("player:%O\nenvironment:%O\n%s"
          ,(this_player())?this_player():0
          ,(this_player() && environment(this_player()))?environment(this_player()):0
          ,res));
...
int valid_write( string file, mixed user, string func )
{
        ...
        &&      objectp(ob) ) {
           i = (int)SECURITY_D->valid_write(file, user, func);

           if(i && sscanf(file,"%*s.c")) {

              CHANNEL_D->do_channel(this_object(),"sys",sprintf("[%s] %s被寫入!",func,file));
           }
           return i;
        }
