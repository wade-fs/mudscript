...
{
  seteuid(getuid());
  set_name("簡易房間編輯器" + RMK_VERSION,
    ({ "simple room maker", "maker" }) );
  set_weight(100);
  set("unit", "個" );
  set("long", @LONG
這是一個用來「簡單地」製造房間的工具﹐讓玩家也能編輯自己的房間。
...
int do_setlong ()
{
  ...
  if (!check_owner(me)) {
    write ("噫? 你怎麼想拆別人的家? 就麻煩你到出入境管理局一趟吧。\n");
    me->move(INN);
...
int do_settle()
{
  ...
  if (!check_owner(me)) {
    write ("噫? 你怎麼想在別人的地盤定居，就麻煩你到出入境管理局一趟。\n");
    me->move(INN);
    destruct (this_object());
    return 1;
  }

  env->set("short", my_name+"與"+couples["name"]+"--愛的小屋");
  env->set("master", ({my_id, couples["id"]}) );

  if (do_saveroom("")) {
    write (my_name+"和"+couples["name"]+"終於安心定居下來了。\n"
     "你看了看四週空空蕩蕩的，好不悽涼，不如開始著手整建一番吧。\n");
    ...
    write ("哇，怎麼不能定居啦，快找人看看風水是不是有什麼問題啊!!\n");
  return 1;
}
