...
void create()
{
	...
	if( clonep() )
		set_default_object(__FILE__);
	        ...
	        set("long","專門用來包紮(bandage <id> )的繃帶。\n");
		set("unit", "條");
		set("value",100);
		set("material", "cloth");
...
int do_bandage(string arg)
{
	...
	if( target->query_temp("medicine/bandaged") )
	  return notify_fail((target=me?"你":target->name())+"的傷口上已經有繃帶裹著了﹐如果你要用來重新包紮﹐請先把它除下(remove)。\n");

	if( me->is_fighting() || target->is_fighting() )
	  return notify_fail("戰鬥中不能包紮。\n");

	if( target->query("kee") == target->query("max_kee") )
	  return notify_fail( (target==me? "你" : target->name())+"並沒有明顯的外傷。\n");

	if( !move(target) ) return 0;

	message_vision("$N細心的用$n替"+(target==me?"自己":target->name())+"裹傷。\n",me,ob);
