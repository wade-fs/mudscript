...
// 連結方向時可以用縮寫
// 造房間時可以自動連接出口
//   並且會依序問問題, 巫師只要回答即可, 有點像問卷
//   如果要用預設值, 直接按 enter
//   如果想放棄編輯, 在輸入的該行, 輸入句點 "." 	<-- 就像在 post
//   如果有不備之處, 請隨時 post
//
//   PS: 本 roommaker 不會因為 saveroom 指令造成蓋掉大部份 function 的錯誤
//   PS: 本檔歡迎拿去修改, 不過請勿流入玩家手中
...
int query_autoload() { object obj = environment(this_object()); if(obj && wizardp(obj)) return 1; }
...
connect <方向> <房間檔名> 	將你目前所在的房間連接一個出口到另一個房間。
                            如果 set auto_connect 的話，會自動在另一個房間連回這間房間。
dconnect <方向>			取消一個出口
...
int do_connect(string arg)
{
  ...
  if(this_player()->query("env/auto_connect"))
  {
    dir = DIR_D->oppdir(dir);
    from = load_object(file);
    exits = from->query("exits");
    printf("%s:%s\n",file_name(environment(this_player())),dir);
    if( mapp(exits) )
      exits[dir] = file_name(environment(this_player()));
    else
      from->set("exits", ([ dir: file_name(environment(this_player())) ]) );
  }
	 write("Ok.\n");
	return 1;
}
...
ROOM_CODE;
  ...
  if( file_size(file)!=-1 ) {
    ...
    input_to("save_room_file", 0, file, room_code);
    return 1;
  }
...
int do_saveroom(string arg)
{
	...
	if( file_size(file)!=-1 ) {
		write("檔案 " + file + " 已存在﹐要刪除舊檔嗎﹖[y/n]");
		input_to("save_room_file", 0, file, str);
		return 1;
	}
