...
int main(object me, string arg)
{
	...
	if( !arg ) return notify_fail("你要打開什麼的鎖﹖\n");
	arg = lower_case(arg);

	if (sscanf (arg, "%s with %s", arg, key) != 2) return
	  notify_fail ("你想用什麼打開門啊。\n");
	...
	if( mapp(doors) ) {
		...
		if( !dir ) return notify_fail("你要打開什麼的鎖﹖\n");
		    ...
		    (string)env->query("need_key/door"))
		{
		  ...
		  {
                      return notify_fail ("這把鑰匙不能開這個門。\n");
		  }
		}

		if( env->unlock_door(dir) ) {
			message_vision("$N聽到喀的一聲, 將" + doors[dir]["name"] + "的鎖打開了。\n", me);
			return 1;
		}
	}
...
指令說明 : 
           這個指令可以讓你打開門。
	   ps. 方向必需使用 east, west...等等, 不能使用 e,w 等簡寫
其他參考 :
           lock，open，close 
HELP
    );
