...
int do_drop(object me, object obj)
{
  ...
  if (obj->query("had_light")) {
       ...
       if ( obj->query("be_light") ) {
        ...
        obj->delete("old_name");     }  
        write (sprintf ("當你把%s丟在地上的時候，這%s%s也漸漸暗淡下來。\n",
                obj->name(), obj->query("unit"), obj->name()));
  }
 
  if(obj->query_temp("focuskee")==1)
    return notify_fail("你的手被一股真氣纏住﹐無法將它丟下。\n");
  if(obj->query("nodrop")==1)
  return notify_fail("此武器吸血中不能丟下。\n");
  if (wizardp(me)) {
    if( obj->is_character() ) {
      if (obj->move(environment(me)) && vis)
        message_vision("$N將$n從背上放了下來﹐躺在地上。\n", me, obj);
      else
        write ("你將"+obj->name(1)+"從背上放了下來﹐躺在地上。\n");
    }
    else {
      if (vis)
        message_vision( sprintf("$N丟下一%s$n，但是它自動銷毀了。\n",
                                 obj->query("unit")), me, obj );
      else
        write ("你丟下一"+obj->query("unit")+obj->name(1)+"，但是它自動銷毀了。\n");
      destruct(obj);
      if (obj) write("你無法將這個物件丟掉。\n");
    }
    return 1;
  }
    ...
    if( obj->is_character() )
      ...
      // 火把
      if (obj->query("had_light")) {
        write (sprintf ("你隨手一丟，這%s%s也漸漸暗淡下來。\n",
