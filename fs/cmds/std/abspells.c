...
{
        int slv,sld;
        slv=me->query("spells/"+arg+"/level");
        sld=me->query("spells/"+arg+"/learned");
        if( !arg ) return notify_fail("你要放棄哪一項法術﹖\n");

        if (!me->query("spells/"+arg+"/") )
        return notify_fail("你並沒有這項法術。\n");
        me->delete("spells/"+arg+"/level",slv);
        me->delete("spells/"+arg+"/learned",sld);
        me->delete("spells/"+arg);
        write("你決定放棄" + to_chinese(arg) + "此項法術。\n");
        return 1;
}
