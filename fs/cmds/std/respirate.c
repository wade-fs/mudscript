...
int main(object me, string arg)
{
	...
	if( me->query("max_atman") >= me->query_skill("magic")*15 && me->query("class")!="dancer" && me->query("class")!="taoist") {
			write("你忽然覺得一陣天旋地轉﹐頭漲得像要裂開一樣﹐似乎靈力的修行已經遇到了瓶頸。\n");
		} else if( (int)me->query("max_atman") >= (int)me->query_skill("magic")*30)
		write("你忽然覺得一陣天旋地轉﹐頭漲得像要裂開一樣﹐似乎靈力的修行已經遇到了瓶頸。\n");
		else {
			write("你的道行提高了﹗\n");
			me->add("max_atman", 1);
