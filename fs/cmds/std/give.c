...
//if npc->set("disable_no_give"), then ppl can give no_give object to npc
//add by bss 093003
inherit F_CLEAN_UP;
...
int main(object me,string arg)
{
        ...
        if(obj->query("no_give")){
          if(!who->query("disable_no_give") || userp(who))
          return notify_fail("這樣東西不能隨便給人。\n");
        }   
         if (wizardp(me))
         { 
         if( !wizardp(who) && wizhood(me)!="(manager)" && wizhood(me) != "(admin)" && wizhood(me) != "(arch)")
         return notify_fail("巫師不能給玩家東西\n");
         log_file("wgi", sprintf("%s(%s) 將%s(%s)給了%s(%s) on %s\n"
        ,me->query("name"),me->query("id"),obj->name(),(string)base_name(obj)
...
int do_give(object me,object obj,object who,int force)
{
  ...
  if( sizeof(all_inventory(who)) >= MAX_CARRY) 
  {
    obj->move(me);
    return notify_fail("對方身上的東西太多了。\n");
  }
  if(!obj) return 1;
