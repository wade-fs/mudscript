...
int main( object me, string arg )
{
	...
	if( !me->query("clan") ) {
		...
		if( !( ob = present( arg, environment(me) ) ) || !living(ob) || !ob->is_character() || ob == me )
			return notify_fail( "您想加入誰的幫派?\n" );
		if( ob->query_temp("c_join") != me->query("id") )
			return notify_fail( "對方並沒有邀請您加入幫派。\n" );
		...
		CHANNEL_D->do_channel( me, "ct",
			"\n\t我 - " + me->query("name") + "(" + me->query("id") + ") 在 " + ob->query("name") + "(" + ob->query("id") + ") 的見證下\n\t向天發誓, 誓死效忠" + me->query("clan/name") + "。\n" );
		return 1;
	}
