...
int main( object me, string arg )
{
	...
	if( environment(me)->query("exits/"+arg) ) {
		write( DIR_D->cdir(arg) + "的出口已經有指定區域了, 確定要取消嗎? [N] :" );
		input_to( "ask_yn", 0, me, arg );
		return 1;
	}
	list = list( CLAN_D->what_clan_area(me) );
	write( "請輸入此出口欲連結之區域號碼: " );
	input_to( "choice_exit", 0, me, arg, list );
        return 1;
}
...
void choice_exit( string choice, object me, string exit, string *list )
{
	...
	if( i<1 || i>j ) {
		list = list( CLAN_D->what_clan_area(me) );
		write( "無此區域, 請重新選擇: " );
		input_to( "choice_exit", 0, me, exit, list );
		return;
	}
