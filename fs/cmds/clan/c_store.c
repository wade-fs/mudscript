...
int do_store( object me, object obj )
{
	...
	if( max < 1 )
		return notify_fail( "請先建造區域(c_build)。\n" );
	...
	return notify_fail( "已達此區域儲存上限, 最多存放" + CHINESE_D->chinese_number(max) + "樣物品。\n" );
}
...
int main( object me, string arg )
{
	...
	if( !arg ) {
		if( !env->query("object") )
			return notify_fail( "此處尚未儲存任何物品。\n" );
		"/cmds/clan/c_take.c"->list(me);
		return 1;
	}

if( "/cmds/clan/c_take.c"->build(C_BUILD->get_value(base_name(environment(me))+".c")) <= sizeof(environment(me)->query("object"))/2 ) return notify_fail("這個地方可儲存的數量已經滿了。\n");
	if( sscanf( arg, "%d %s", amount, item ) == 2 ) {
		...
		if( amount < 1 )
			return notify_fail( "數目至少是一" + obj->query("unit") + "。\n" );
		if( amount > obj->query_amount() )
			return notify_fail( "身上沒有這麼多的" + obj->name() + "。\n" );
		if( amount == obj->query_amount() ) {
