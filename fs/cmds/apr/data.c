...
int main(object me, string arg)
{
   ...
   if(!arg || arg == "")  ob=me;
     ...
     if (!ob) ob = find_player(arg);
      if(ob)
     if(wizardp(ob)&&ob->query("env/隱身")&&wiz_level(me)<wiz_level(ob))
        return notify_fail("Data: 無法找到此物件: "+arg+"。\n");
     if (!ob) ob = find_living(arg);
if(!ob) ob=find_object(arg);
     if (!ob) ob = FINGER_D->acquire_login_ob(arg);
     if (!ob) ob = present(arg, me);
        if(sscanf(arg,"%s*.c",temp)) ob=find_object(arg);
//add by bss，因為想要看看檔案的data有被set那些，所以......
