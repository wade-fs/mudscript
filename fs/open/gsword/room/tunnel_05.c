...
void create () {
        ...
        set("light_up",-1);
        set("exits", ([
            "north" :  __DIR__"tunnel_06",
			"south" :  __DIR__"tunnel_04",   
      ]));
        setup();
}
...
void do_damage(object me)
{
  ...
  if(!me->query_temp("quest/return_sha") || me->query_temp("quest/return_sha")<7)  //防止玩家以不正當方式進入
  {
   ...
   if(!eq)
   {
    ...
    message_vision(HIG + " $N身上的"+eq->query("name")+HIG"英氣逼人，及時為主人擋下這致命一擊，隨即灰飛湮滅。\n" + NOR,me);
	destruct(present("Xstrike-Hands",me));
          }
   return ;
}
