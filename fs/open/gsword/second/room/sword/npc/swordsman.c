...
set("chat_msg_combat",({
        ...
        carry_object("/open/gsword/obj1/swordball");
}
void init() {
  object obj;
  ::init();
...
{
 winner->delete_temp("swordquest/fball");
...
string ask_fireball()
{
 if(this_player()->query_temp("swordquest/ball")!=3)
 {
 ...
 else if (!this_player()->query("swordquest/findball"))
  {
   ...
   return ("嗯，你現在已經回到狂想世界了，記住，你要找的是連陽劍晶珠，而你的時間只有一個時辰，屆時我會將你召回!!");
  }
 else return ("你不是找過了!!");
...
string ask_waterball()
{
 ...
 else if ((this_player()->query_temp("swordquest/findball"))<7 ||this_player()->query("swordskill/fsword8")==1)
