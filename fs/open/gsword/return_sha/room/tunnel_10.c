...
void create () {
        ...
        set("light_up",-1);
        set("exits", ([
            "out" :  "/open/gsword/room1/blood1/r10",
			"eastdown" :  __DIR__"tunnel_09",   
      ]));
        setup();
}
...
{
 if(dir == "out" && who->query_temp("quest/return_sha") > 7)
  {
    return notify_fail("你已和血魔反目，再過去就太危險了，還是回頭吧。\n");
  }
  return 1;
}
...
void do_damage(object me)
{
  ...
  if(!me->query_temp("quest/return_sha") || me->query_temp("quest/return_sha")<7)  //防止玩家以不正當方式進入
  {
   ...
   if(!eq)
   {
    ...
    message_vision(HIG + " $N身上的"+eq->query("name")+HIG + "龍氣瀰漫，及時為主人擋下這致命一擊，隨即灰飛湮滅。\n" + NOR,me);
