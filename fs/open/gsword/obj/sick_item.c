...
void create()
{
  ...
  set("long",
    "此物乃蜀中將軍自東海漩渦採來之神藥，專治藥性。此藥需經藥師調配(check_item\n"+
    "ice item )，方得食用(eat pill)。\n");
  set("unit", "棵");
  set("base_unit", "棵");
  set("value",2500);
  set("base_weight", 50);
...
int check_item(string arg)
{
  ...
  if( (arg=="item" || arg=="ice item") )
  {
    object user,ob;
    user = this_player();
    if(user->query("class")!="doctor" && !wizardp(user))
...
//      message_vision("$N試著將冰精製成藥珠。\n", user);
      ...
      if( user->query_skill("cure",1) < 120 )
        user->improve_skill("cure",random((int)cure)); //估計值為0~32
      else
