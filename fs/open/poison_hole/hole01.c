...
int use_rope(string arg)
{
  object obj,me;
  me = this_player();
  if(!arg || (arg != "rope east" && arg != "rope west"))
  {
    write("你要使用什麼？可以使用 use rope east 或是 use rope west 指定方向\n");
    return 1;
  }
  ...
  if(arg == "rope west")
  {
    me->move(__DIR__"hole02.c");
  }
  else if(arg == "rope east") 
  {
    me->move(__DIR__"hole03.c");
  }
  else
  {
 message_vision("$N一個不小心，直接摔落山谷，並且發出啊啊啊啊啊啊的慘叫聲。\n",this_player());
