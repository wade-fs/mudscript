#include <ansi.h>
int up(object me);

void init()
{
	object me,ob,obj,mob,room;
	int kee1,j;
	me=this_player();
	j=random(3);

	if( me->query_temp("magic-manor/kill-soil04") >= 20 )
	{
	remove_call_out("up");
	call_out("up",1,me);
	}else{
	  if( !me->query("soil-monster") && !me->is_corpse() && me->is_character() && living(me) )
	  {
	  kee1=me->query("eff_kee");
	    if( random(600) > me->query_skill("force") )
	    {
	    message_vision(HIR + "$N" + HIR + "一時運功\不足，慘被強烈的音波給震傷了!!\n" + NOR,me);
	    me->receive_wound("kee",kee1/50);
	    COMBAT_D->report_status(me,1); 
	    }else{
	    message_vision(HIW + "$N" + HIW + "運足內家功\力，抵擋住了音波攻擊!!\n" + NOR,me);
	    }   
	  }

	  if( !present("soil monster",environment(me)) && !present("soil beast",environment(me)) )
	  {
	    switch(j)
	    {
	    case 0:
	    mob = new(__DIR__"npc/soil-beast01");
	    mob->move(environment(me));
	    break;

	    case 1:
	    mob = new(__DIR__"npc/soil-monster04");
	    mob->move(environment(me));
	    mob = new(__DIR__"npc/soil-monster04");
	    mob->move(environment(me));
	    break;

	    case 2:
	    mob = new(__DIR__"npc/soil-monster04");
	    mob->move(environment(me));
	    mob = new(__DIR__"npc/soil-monster04");
	    mob->move(environment(me));
	    break;
	    }
	  }
	}
	return ;
}

int up(object me)
{
	me=this_player();
	message_vision(YEL + "土色的靈氣包圍漸漸褪去，一個向上的樓梯漸漸成形在$N眼前。\n" + NOR,me);
	me->move("/open/magic-manor/soil/up04");
}
