...
object ob,me;
	...
	return notify_fail(""+ob->name(1)+"無法在此聚集靈氣。\n");
        ...
        if( me->query_temp("magic-manor/gather-water") )
...
int do_scatter(string arg)
{
	...
	ob->delete("no_save",1);
        ...
        ob->set("name","靈氣水珠");
        me->delete_temp("magic-manor/gather-water",1);
	me->delete_temp("magic-manor/water-shield",1);
	return 1;
}
