...
object ob,me;
	...
	return notify_fail(""+ob->name(1)+"無法在此聚集靈氣。\n");
        ...
        if( me->query_temp("magic-manor/gather-fire") )
...
int do_scatter(string arg)
{
	...
	ob->delete("no_save",1);
        ...
        ob->set("name","靈氣火珠");
        me->delete_temp("magic-manor/gather-fire",1);
	me->delete_temp("magic-manor/fire-shield",1);
	return 1;
}
...
int act9(object ob,object me)
{
	...
	message_vision(HIW + "只見$N" + HIW + "已吸足了靈氣，$N" + HIW + "周身泛起強烈光芒。\n" + NOR,ob);
        ob->set("name",HIW + "(聚靈) " + NOR+HIR + "靈氣火珠" + NOR);
        me->set_temp("magic-manor/gather-fire",1);
	me->set_temp("magic-manor/fire-shield",1);
	return 1;
}
