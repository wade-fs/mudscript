...
void create()
{
        ...
        {
	set("long", @LONG
相傳如果要進入五行靈塔，就必需要攜帶的寶物，靈氣木珠所散
發出來的靈氣，足以克制五行之中的土靈之氣，可以減少具有傷害的
靈氣所帶來之傷害。靈氣木珠必需要聚靈之後，才會開啟其作用，若
尚未聚靈的靈氣木珠是無任何作用的。
...
object ob,me;
	...
	return notify_fail(""+ob->name(1)+"無法在此聚集靈氣。\n");
        ...
        if( me->query_temp("magic-manor/gather-wood") )
...
int do_scatter(string arg)
{
	...
	ob->delete("no_save",1);
        ...
        ob->set("name","靈氣木珠");
        me->delete_temp("magic-manor/gather-wood",1);
	me->delete_temp("magic-manor/wood-shield",1);
	return 1;
}
...
int act9(object ob,object me)
{
	...
	message_vision(HIW + "只見$N" + HIW + "已吸足了靈氣，$N" + HIW + "周身泛起強烈光芒。\n" + NOR,ob);
        ob->set("name",HIW + "(聚靈) "NOR+HIG"靈氣木珠" + NOR);
        me->set_temp("magic-manor/gather-wood",1);
	me->set_temp("magic-manor/wood-shield",1);
	return 1;
}
