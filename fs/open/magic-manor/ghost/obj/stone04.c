...
void create()
{
	set_name("封魂石",({"fon stone","fon","stone"}));
...
int check(object ob)
{
	...
	room->add("exits/takeroad","/open/magic-manor/ghost/rest05");
	call_out("msg1",60,ob);
...
int msg1(object ob)
{
	...
	bname = base_name(environment(glist[l]));
	  if( bname == "/open/magic-manor/ghost/rest04" )
	  {
	  tell_object(glist[l],HIR + "\n只見到封魂石漸漸的崩碎消失!!\n" + NOR);
	  tell_object(glist[l],HIR + "\n而你"NOR+HIR"漸漸被一道靈光包圍傳送到另一個地方!!\n" + NOR);
	  glist[l]->move("/open/magic-manor/ghost/rest05");
	  }
        }
	...
	for (j=0 ; j < i ; j++)
	{
	  if( !enemy[j] ) continue;
	  {
	    ...
	    tell_object(enemy[j],HIR + "\n而你"NOR+HIR"漸漸被一道靈光包圍傳送到另一個地方!!\n" + NOR);
	    enemy[j]->move("/open/magic-manor/ghost/rest05");
	    }
	  }
	}
