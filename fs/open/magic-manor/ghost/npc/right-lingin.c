...
void create()
{
  set_name("右靈晉",({"right lingin","right","lingin"}));
  set("long",@LONG
...
LONG);
  ...
  set("functions/extra-kee/level",100);
...
int accept_kill(object ob)
{
  object me,left;
  me=this_object();
  left=present( "left linr",environment(me) );

  if( !present( "left linr",environment(me)) || !living(left) ) return 1;
  else {
    message_vision(HIY"\n右靈晉說：$N想死就來嚐嚐我和左靈兒的極招吧！\n"NOR,me);
    left->kill_ob(ob);
    me->delete_temp("is_busy");
    me->delete_busy();
    command("wield all");
    command("wear all");
    command("perform unarmed.ice-fingers");
    return 1;
  }
}
