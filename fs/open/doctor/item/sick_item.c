...
{
  seteuid(getuid());
  set_name("東海冰精", ({"ice item","item"}) );
  set("long",
    "此物乃蜀中將軍自東海漩渦採來之神藥，專治藥性。須藥師調配(check_item ice item)，\n"+
    "方得食用(eat pill)。\n");
  set("unit", "棵");
  set("base_unit", "棵");
  set("no_sell",1);
...
//      message_vision("$N試著將冰精製成藥珠。\n", user);
      ...
      if( user->query_skill("cure",1) < 120 )
        user->improve_skill("cure",random((int)cure*2)); //估計值為0~64
      else
        user->improve_skill("cure",(int)random((int)cure/2)+1); //估計值為1~17
      add_amount(-1);
    } else
      message_vision("$N的醫術太低，無法將冰精製成藥珠。\n", user);
    return 1;
  }
  return 0;
}
