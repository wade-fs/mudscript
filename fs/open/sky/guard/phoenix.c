...
void create()
{
  set_name(HIY"琉凰"NOR ,({"phoenix guard", "guard"}) );
  set("race", "野獸");
  set("nickname", HIG"靈隨光"NOR);
  set("age",1032);
...
void heart_beat()
{
  ...
  if( me->is_fighting() )
  {
    if( j > 50 && j < 65 )
    {
      ...
      {
        message_vision("$N"HIY"身上發出一道靈光向$n"HIY"捲了過去，$n"HIY"走避不及被靈光纏住了!!!\n"+NOR,me,enemy[i]);
        enemy[i]->receive_damage("kee", 350, me);
        enemy[i]->apply_condition("magickee",random(4)+5);
    ...
    if( j > 65 && j < 75 )
    {
      ...
      if(environment(me) == environment(enemy[i]))
      {
        message_vision("$N"HIY"身上發出一道靈光向$n"HIY"飛了過去，$n"HIY"走避不及被靈光纏住了!!!\n"+NOR,me,enemy[i]);
        enemy[i]->receive_damage("kee", 350, me);
        enemy[i]->apply_condition("magickee",random(4)+5);
    ...
    if( j > 75 && j < 80 )
    {
      ...
      for(i=0;i< sizeof(enemy);i++)
      {
        ...
        if(environment(me) == environment(enemy[i]))
        {
          message_vision("$N"HIY"身上發出數道靈光向$n"HIY"襲了過去，$n"HIY"走避不及被靈光纏住了!!!\n"+NOR,me,enemy[i]);
          enemy[i]->receive_damage("kee", 350, me);
          enemy[i]->apply_condition("magickee",random(4)+5);
          enemy[i]->start_busy(1);
          COMBAT_D->report_status(enemy[i], 1);
    ...
    if( j > 80 && j < 84 )
    {
      ...
      for(i=0;i< sizeof(enemy);i++)
      {
        ...
        if(environment(me) == environment(enemy[i]))
        {
          message_vision("$N"HIY"身上發出數十道靈光向$n"HIY"撲了過去，$n"HIY"走避不及被靈光纏住了!!!\n"+NOR,me,enemy[i]);
          enemy[i]->receive_damage("kee", 350, me);
          enemy[i]->apply_condition("magickee",random(4)+5);
          enemy[i]->start_busy(2);
          COMBAT_D->report_status(enemy[i], 1);
