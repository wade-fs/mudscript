...
void create()
{
 set_name("刀無盡",({"blade god","blade","god"}));
 set("long",@LONG
天靈地界的刀中戰神，用刀技能一絕，無人能匹敵，性格兼具
善與惡之間。
LONG);

  set("gender","男性");
  set("class","blademan");
  set("nickname",HBBLU+HIG"雙刀傳奇"NOR);
...
void init()
{
  if( !present("attribute-blade",this_object()) )
  {
...
//變換門派技能部份
  if( k == 1 || k == 11 )
  {
    ...
    me->set("family/family_name","金刀門");
    me->set("cs",2);
    command("enable parry gold-blade");
    command("enable blade fast-blade");
    map_skill("parry","gold-blade");
    map_skill("blade","fast-blade");
  }
  ...
  if( k == 9 || k == 19 )
  {
    ...
    me->set("cs",6);
    command("enable parry dragon-blade");
    command("enable blade dragon-blade");
...
void unconcious()
{
  ...
  if( present("attribute-blade",me))
  {
    destruct(present("attribute-blade",me) );
  }
  if(!winner)
  {
    :: unconcious();
...
//  i=random(50);
  ...
  if( !me->query("have_item") )
  {
    if( random(100) == 50 )
    {
      ob1=new("/open/sky/obj11/black-crystal.c");
