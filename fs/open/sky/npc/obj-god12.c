...
void create()
{
  set_name("嬂云姬",({"needle god","needle","god"}));
  set("long",@LONG
天靈地界中的針術名家，一雙巧手能將見過的景色織成一幅絕
美的畫面，其擅長的也是針術攻擊，夢靈轉功力已經到了深不
可測境界，絕美的外表下，也藏著不可思議的能為。
LONG);
  ...
  set_skill("god-needle",150);
  set_skill("mamagic",500);
  ...
  map_skill("stabber","god-needle");
  map_skill("magic","mamagic");
  ...
  set("functions/magicpower/level",150);
  ...
  set_temp("ma-needle",1);
  set_temp("thunder_power",10000);
...
void init()
{
  if( !present("attribute-needle",this_object()) )
  {
    command("change_attr needle");
    command("wield attribute-needle");
  }
...
void heart_beat()
{
  object *enemy,me=this_object(),eq,*target;
  ...
  if( !present("attribute-needle",me) )
  {
    command("change_attr needle");
    command("wield attribute-needle");
  }
  ...
  if( me->query("have_busy") >= 10 )
  {
    me->delete_temp("is_busy");
    me->delete_busy();
    me->receive_curing("kee",1000+random(1000));
    me->receive_heal("kee",1000+random(1000));
    me->receive_curing("gin",1000+random(1000));
    me->receive_heal("gin",1000+random(1000));
    me->receive_curing("sen",1000+random(1000));
    me->receive_heal("sen",1000+random(1000));
    me->add("force",5000);
    me->clear_condition();
    me->delete("have_busy");
  }
...
void unconcious()
{
  ...
  if( present("attribute-needle",me))
  {
    destruct(present("attribute-needle",me) );
  }
...
//  i=random(50);
  ...
  if( !me->query("have_item") )
  {
    if( random(100) == 50 )
    {
      ob1=new("/open/sky/obj12/banana-doll.c");
