...
void create()
{
  set_name("劍玅藏",({"bsword god","bsword","god"}));
  set("long",@LONG
擅於血魔一脈的劍術，血魔霸劍式已經到了神一般的境界，劍
式一旦施展開來，非死不能罷休，雖為天靈地界的一神祇，但
隱隱帶著一點魔性。
LONG);
  ...
  set("functions/bloodkee/level",120);
...
void init()
{
  if( !present("attribute-sword",this_object()) )
  {
...
int accept_kill(object ob)
{
  ...
  kill_ob(ob);
  command("cmd god_kee");
  command("perform blood_sword.bloodkee "+ob->query("id"));
  return 1;
}
...
//技能攻擊部份
  ...
  {
    j=random(i);
    command("perform blood_sword.bloodkee "+enemy[j]->query("id"));
  }

  if( me->is_fighting()&& me->is_busy() && bb > 15 )
  {
    me->delete_temp("is_busy");
    me->delete_busy();
    me->delete_temp("no_power_f");
...
//  }
  if( present("attribute-sword",me))
  {
    destruct(present("attribute-sword",me) );
  }
  if(!winner)
  {
    :: unconcious();
...
//  i=random(50);
  ...
  if( !me->query("have_item") )
  {
    if( random(100) == 50 )
    {
      ob1=new("/open/sky/obj6/forest-diamond.c");
