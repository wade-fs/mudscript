...
{ 
        if( str == "all" || str == "cloak" || str == "water light cloak" )
        {
        ::wear();
          if( query("equipped") ) 
          {
          user = this_player();
          message_vision(HBBLU+HIC + "水色天光" + NOR+HIW + "在蕭瑟的秋水下﹐讓天際泛起了陣陣炫目的光彩﹐耀眼奪目.\n" + NOR,user);
          set_heart_beat(1);
          }
        }
}
...
int do_remove(string str)
{
   if(str=="cloak" || str=="all" || str=="water light cloak")
     if( query("equipped") )
     {
        message_vision(HIW + "耀眼炫目的光彩不再﹐" + HBBLU+HIC + "水色天光" + NOR+HIW + "漸漸穩沒在蕭瑟的秋水下.\n" + NOR,user);
        set_heart_beat(0);
     }   
}                       
        ...
        if( user->is_fighting() && query("equipped") )
        {
          ...
          j=random(sizeof(enemy));
            ...
            message_vision(sprintf(HIM + "－＝≡ " + HBBLU+HIC + "驚 － 天 － 動 － 地 － 式" + NOR+HIY + " ～◎～ " + HBBLU+HIC + "狂 － 滅 － 天 － 九 － 絕" + NOR+HIM + " ≡＝－\n\n" + NOR),user,enemy[j]);
              ...
              enemy[j]->receive_damage("kee",400);
              COMBAT_D->report_status(enemy[j]);
              break;

              case 1:
              message_vision(HIC + "狂滅天九絕第二式" + HBCYN+HIC + "【 天風捲千鈞 】" + NOR+HIC + "龐然巨大的龍捲颶風狂吹亂掃，向$n襲擊.\n" + NOR,user,enemy[j]);
              enemy[j]->receive_damage("gin",240);
              enemy[j]->receive_damage("sen",240);
              enemy[j]->receive_damage("kee",600);
              COMBAT_D->report_status(enemy[j]);
              break;

              case 2:
              message_vision(HIW + "狂滅天九絕第三式" + HBWHT+HIW + "【 天龍舞飛旋 】" + NOR+HIW + "無數的龍氣聚然成形激射狂旋，向$n飛舞.\n" + NOR,user,enemy[j]);
              enemy[j]->receive_damage("gin",260);
              enemy[j]->receive_damage("sen",260);
              enemy[j]->receive_damage("kee",800);
              COMBAT_D->report_status(enemy[j]);
              break;

              case 3:
              message_vision(HIB + "狂滅天九絕第四式" + HBWHT+HIB + "【 天水月流轉 】" + NOR+HIB + "朦朧的月色若隱若現輕盈流轉，向$n盈繞.\n" + NOR,user,enemy[j]);
              enemy[j]->receive_damage("gin",280);
              enemy[j]->receive_damage("sen",280);
              ...
              enemy[j]->receive_damage("gin",300);
              enemy[j]->receive_damage("sen",300);
              enemy[j]->receive_damage("kee",1200);
              COMBAT_D->report_status(enemy[j]);
              break;
              ...
              enemy[j]->receive_damage("sen",320);
              enemy[j]->receive_damage("kee",1400);
              COMBAT_D->report_status(enemy[j]);
              break;

              case 6:
              message_vision(HIM + "狂滅天九絕第七式" + HBMAG+HIM + "【 天靈箭幻舞 】" + NOR+HIM + "大地的靈氣如幻箭般激射四起，向$n狂射.\n" + NOR,user,enemy[j]);
              enemy[j]->receive_damage("gin",340);
              enemy[j]->receive_damage("sen",340);
              enemy[j]->receive_damage("kee",1600);
              COMBAT_D->report_status(enemy[j]);
              break;

              case 7:
              message_vision(HIR + "狂滅天九絕第八式" + HBRED+HIR + "【 天火怒燎原 】" + NOR+HIR + "天神怒火由四面八方襲捲而至，向$n焚燒.\n" + NOR,user,enemy[j]);
              enemy[j]->receive_damage("gin",360);
              enemy[j]->receive_damage("sen",360);
              enemy[j]->receive_damage("kee",1800);
              COMBAT_D->report_status(enemy[j]);
              break;

              case 8:
              message_vision(HIC + "狂滅天九絕第九式" + HBBLU+HIC + "【 天冰寒星雨 】" + NOR+HIC + "萬點的寒星如驟雨般破空急降，向$n暴降.\n" + NOR,user,enemy[j]);
              enemy[j]->receive_damage("gin",380);
              enemy[j]->receive_damage("sen",380);
              enemy[j]->receive_damage("kee",2000);
              COMBAT_D->report_status(enemy[j]);
              break;

              case 9:
              message_vision(HIR + "狂滅天九絕第一式" + HBRED+HIY + "【 天翔奔雷破 】" + NOR+HIR + "由天降下的無數雷球電氣四射，向$n狂襲.\n" + NOR,user,enemy[j]);
              enemy[j]->receive_damage("kee",100);
              COMBAT_D->report_status(enemy[j]);
              message_vision(HIC + "狂滅天九絕第二式" + HBCYN+HIC + "【 天風捲千鈞 】" + NOR+HIC + "龐然巨大的龍捲颶風狂吹亂掃，向$n襲擊.\n" + NOR,user,enemy[j]);
              enemy[j]->receive_damage("kee",100);
              COMBAT_D->report_status(enemy[j]);
              ...
              COMBAT_D->report_status(enemy[j]);
              message_vision(HIC + "狂滅天九絕第九式" + HBBLU+HIC + "【 天冰寒星雨 】" + NOR+HIC + "萬點的寒星如驟雨般破空急降，向$n暴降.\n" + NOR,user,enemy[j]);
              enemy[j]->receive_wound("gin",400);
              enemy[j]->receive_wound("sen",400);
              enemy[j]->receive_wound("kee",2000);
