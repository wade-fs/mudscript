		...
		if( war || alert ) {
			if( kill )
				...
				if( !who->query_temp("clan/invade") ) {
					...
					if( cid_who )
						MSG->npc_ct( npc,
"$HIR$注意!! 發現$HIM$" + cname_who + "$HIR$「$HIM$" + who->query("clan/title") + "$HIR$」$HIM$" + who->query("name") + "$HIR$($HIM$" + who->query("id") + "$HIR$)非法闖入" + (environment(npc))->query("short") + "$HIR$!!$NOR$" );
					else
						MSG->npc_ct( npc,
"$HIR$注意!! 發現$HIM$" + who->query("name") + "$HIR$($HIM$" + who->query("id") + "$HIR$)非法闖入" + (environment(npc))->query("short") + "$HIR$!!$NOR$" );
				}
			}
		}
...
//	ally      = member_array( cid_who, npc_ally )+1;
	...
	if( CLAN_D->is_clan_room(npc) )	{
		if( war )	{
			if( cid_who )
				MSG->npc_ct( npc,
"$HIM$" + cname_who + "$HIR$「$HIM$" + who->query("clan/title") + "$HIR$」$HIM$" + who->query("name")+ "$HIR$($HIM$" + who->query("id") + "$HIR$)已攻破" + env->query("short") + "$HIR$, 請火速支援!!$NOR$" );
			else
				MSG->npc_ct( npc,
"$HIM$" + who->query("name")+ "$HIR$($HIM$" + who->query("id") + "$HIR$)已攻破" + env->query("short")+ "$HIR$, 請火速支援!!$NOR$" );
		}
		else if( alert && cname_who!=cname_npc && !ally ) {
			if( cid_who )
				MSG->npc_ct( npc,
"$HIR$注意!! 發現$HIM$" + cname_who + "$HIR$「$HIM$" + who->query("clan/title") + "$HIR$」$HIM$" + who->query("name") + "$HIR$($HIM$" + who->query("id") + "$HIR$)非法闖入" + env->query("short") + "$HIR$!!$NOR$" );
			else
				MSG->npc_ct( npc,
"$HIR$注意!! 發現$HIM$" + who->query("name") + "$HIR$($HIM$" + who->query("id") + "$HIR$)非法闖入" + env->query("short") + "$HIR$!!$NOR$" );
		}
	}
