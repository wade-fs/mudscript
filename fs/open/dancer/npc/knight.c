...
{
//	set("title","神秘劍客");
	set_name("神秘劍客", ({"mystery swordsman","swordsman"}));
	set("age",75);
	set("long","謎一般的神秘男子。\n");
	set("gender","男性");
	set("str",30);
...
//	set("sen",150000);
	...
	set("dev_obj/"+dev[random(sizeof(dev))],101+random(69300));
...
*/	  enemy = me->query_enemy();	
	    ...
	    {
	      if( !me->query("have_exp") )
	      {
	      me->set("combat_exp",enemy[i]->query("combat_exp")+random(55555));
	      me->set("have_exp",1);
	      }
	    }
          }
...
{    
  ...
  if(winner && !winner->query_temp("not_robot") && random(1000)==0) {
    ob=new("/open/doctor/doctor_book/obj/scrap");
    ob->set("sort","角");
    ob->move(winner);
    message_vision(RED"\n從神秘劍客的身上掉下了一塊靈魂碎片!!\n"NOR,winner);
  }
  else if(winner && winner->query_temp("not_robot") && random(2000)<200) {
    ob=new("/open/doctor/doctor_book/obj/scrap");
    ob->set("sort","角");
    ob->move(winner);
    message_vision(RED"\n從神秘劍客的身上掉下了一塊靈魂碎片!!\n"NOR,winner);
	...
	write_file("/log/get_war/swordsman",sprintf("%s(%s)	得到一百點戰功\於 %s\n",
	winner->name(1),winner->query("id"),ctime(time())));
