	...
	*/
	if( wizardp(me) || wizardp(victim) )
        ...
        case "鞭傷":
                ...
                else return "結果一鞭命中,只聽見咯啦一聲,，n哀號倒地$l筋斷骨裂,鮮血狂湧不止﹗﹗\n";
                break;
        default:
                if( !type ) type = "傷害";
...
{
if(damage > 0.4*victim->query("max_kee"))
{
message_vision(HIR + "\n$N的強大攻擊力強行突破了$n的" + HIW + "幻夢之翼" + HIR + "，直接對$n本體造成傷害。\n" + NOR,me,victim);
...
victim->delete_temp("can_contour");
      ...
      if(!me->query("env/my_skill")) {    //修正使用自己的招式時..不出現原有招式的特功!!!!
   if(me->query("force") > 1) {
         ...
         if(userp(killer)) //killer是玩家才執行下面的
          {
           ...
           { //殺死的人是pker或ckill的話就不算pk
            ...
            tell_room(killer,HIW + "你為民除害,殺了殺人兇手,得到了一筆酬金。\n" + NOR);
                ...
                // bls = 10;
                bls = 0;
	switch( random(3) ) { // by ACKY
	case 1:
		CHANNEL_D->do_channel( this_object(), "dead",
			sprintf( "爽快的說道：「%s被%s給徹底轟下了。」",victim->name(1),killer->name(1) ) );
		break;
	case 2:
		CHANNEL_D->do_channel( this_object(), "dead",
			sprintf( "爽快的說道：「%s被%s殺死了。」",victim->name(1),killer->name(1) ) );
		break;
	default:
		CHANNEL_D->do_channel( this_object(), "dead",
			sprintf( "爽快的說道：「%s被%s給徹底轟下了。」",victim->name(1),killer->name(1) ) );
