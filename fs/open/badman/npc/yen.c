...
string do_ask1()
{
  ...
  if (me->query("quests/badman_license")) {
    ...
    if (new("/autoload/badman/license.c")->move(me))
...
{      
  ...
  if (new("/autoload/badman/license.c")->move(who)) {
    if (random(2))
...
return notify_fail("你已練習的比我還強了？我已沒東西教你了！\n");
if( me->query("potential") - me->query("learned_points") < 10)
return notify_fail("你潛能不夠還想學??\n");
   if( me->query("get_badsword") != 1 )
   {
   return notify_fail("你還未學得初級劍法，我教你也沒用ㄚ\n");
   }
   else
   {
   write("你專心的學習著魔日劍法\n");
