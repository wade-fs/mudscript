...
薄紗矇住了臉！\n");
	...
	"廢物":"武功盡失的你，跟廢物不是沒兩樣嗎？...雖然不是真的廢掉了...不過也差不多...\n",
	"白骨精":"就是你在一堆骨骸中遇到的那個騙人精...只會用幻術騙人的幻魔！\n",
	"騙":"唉～毛頭小子，你只是被他的幻術給蒙蔽了心靈，不過，強大的幻術催眠，也會讓人變成廢物...\n",
	"幻術催眠":"沒錯，那個白骨精只是用幻術催眠，讓你自認為武功盡失，還是有方法可以恢復，不過...\n",
...
int accept_object(object me,object obj)
{
	...
	if(obj->query("id")=="glass grass")
	{
		tell_object(me,"夢婆說：很好，再加上1顆鑽石當手工費！\n");
		me->set_temp("/area/forest/gress",1);
	}
	
	if(obj->query("id")=="diamond")
	{
		if(me->query_temp("/area/forest/gress"))
		{
			me->set("/area/forest/gress",1);
