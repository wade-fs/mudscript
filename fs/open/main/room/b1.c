// Room: /open/main/room/b1.c

#define SAN_GUAN "/open/main/npc/gamble_man"
inherit ROOM;

void create()
{
	set("short", "橋");
	set("long", @LONG
這是出西蜀關的唯一要道, 為了維護治安, 不時會有民安隊員
出現, 偶而也會有駐軍, 只是人稱一到西蜀魂歸時, 關外的險惡早
已經嚇壞的人們, 所以這座橋也時時發出吱吱的聲響, 似乎岌岌可
危, 橋下是出名的大河瀾滄江, 一掉下去恐怕命都沒了. 還是不要
在這兒逗留為上.
LONG
	);
	set("exits", ([ /* sizeof() == 2 */
  "northeast" : __DIR__"r18",
  "west" : __DIR__"b1",
]));

  set("objects", ([
	SAN_GUAN:1,
  ]) );

	setup();
}

void init()
{
  add_action("do_search", "search");
}

int do_search(string arg)
{
  object	me=this_player();
  int		query;

  if (((query=me->query_temp("quests/gamble_book")) < 3) &&
      !me->query("quests/gamble_book")) return 0;
  if (arg)
    if (query > 3) return notify_fail("你找來找去也沒發現什麼特殊的地方。\n");
    else return 0;

  me->set_temp("quests/gamble_book", 4);
  message_vision("$N找來找去終於發現一條往下的路。\n", me);
  set("exits/down", __DIR__"gamble_book");
  return 1;
}

int valid_leave(object me, string dir)
{
  if (dir != "down") {
    this_object()->delete("exits/down");
    return 1;
  }
  if (!me->query("quests/gamble_book") &&
       me->query_temp("quests/gamble_book") < 4) return 0;
  if (this_object()->query("exits/"+dir)) return 1;
  else return 0;
}
