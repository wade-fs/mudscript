...
//修正使用者不存在會出錯的問題 by blazakira 2011/2/22
...
int do_fire()
{
  ...
  if( user->is_fighting() && query("equipped") )
  {
    ...
    { 
      if( random(250)+max > enemy[i]->query_skill("dodge") )
      {
        ...
        else if( sp < 20 )
        {
          message_vision(HIR+"$N手中長劍往地上一指,熊熊火燄倏然從地底冒"
            +"出,燃燒著$n!!!\n"+NOR,user,enemy[i]);
          pp=sp*2;      
          enemy[i]->apply_condition("burn",(int)sp*3/2);
        }
        else if( sp < 30)
        {
      ...
      {
        message_vision(HIR+"$N嘗試招喚火燄...但是沒有成功\...\n"
          +NOR,user);      
        sp=sp*0.7;
      }
    }
