...
"$N割開心臟吸乾了其中黑色的血水之後, 發出了一聲淒厲的慘嚎, 身子慢慢倒了下去...\n", me);
        me->die();
        if(userp(me)) log_file("mogi/poison", sprintf("%s(%s) 二次誤食千墳之水葛屁 on %s\n",me->query("name"),me->query("id"), ctime(time()) ));
	add_amount(-1);
	return 1;
                                       }
...
"$N割開心臟吸乾了其中黑色的血水, 突然覺得腦中一陣劇痛, 隨即七竅噴出黑血, 入滅之毒猛然發作！\n", me);
        ...
        if(userp(me)) log_file("mogi/poison", sprintf("%s(%s) 誤食假千墳之水 on %s\n",me->query("name"),me->query("id"), ctime(time()) ));
	add_amount(-1);
	return 1;
                                       }
       else {
...
"$N割開心臟吸乾了其中黑色的血水, 忽然頭痛欲裂, 臉上霎時全無血色！\n", me);
        ...
        if(userp(me)) log_file("mogi/poison", sprintf("%s(%s) 中入滅之毒(誤食) on %s\n",me->query("name"),me->query("id"), ctime(time()) ));
	add_amount(-1);
	return 1;
        }    	
}
