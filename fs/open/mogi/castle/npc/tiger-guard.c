...
void create() {
	...
	set_name("白虎守衛",({"tiger guard","guard"}));
	set("long",
	    "\n他是鎮守白虎室的厲害妖魔。\n");
...
{
return notify_fail("白虎守衛說道 : 我沒空陪你玩。\n");
}
...
{    
	...
	j=random(-1);
	  ...
	  {      
	  new("/open/sky/obj7/tiger-order")->move(environment(winner));
	  message_vision(HIM"\n從白虎守衛的身上掉下了一件奇怪的東西!!\n"NOR,winner);
          write_file("/log/sky/obj7/tiger_order",sprintf("%s(%s) 讓白虎守衛掉下了白虎結令於 %s\n",
