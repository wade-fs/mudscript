...
{
      message_vision(HIW+"道道閃光從雷龍口中疾吐而出 ,正朝著$N迅速直劈而去!!\n"+NOR,enemy[i]);
      message_vision(HIC+"$N閃躲不及 ,被燒得的全身焦黑。\n"+NOR,enemy[i]);
     COMBAT_D->report_status(enemy[i], 1);
     enemy[i]->receive_damage("kee", 200);     
  COMBAT_D->report_status(enemy[i], 1);
 
}else{
      message_vision(HIW+"道道閃光雷龍的口中疾吐而出 ,正朝著$N迅速直劈而去!!\n"+NOR,enemy[i]);
     message_vision(HIC+"只見$N飛身側閃 ,輕輕避開閃電的攻擊...\n"+NOR,enemy[i]);
