...
{
      message_vision(HIR+"紅色的烈燄自火龍口中湧湧而出 ,正朝著$N迅速狂捲而去!!\n"+NOR,enemy[i]);
      message_vision(HIY+"$N閃躲不及 ,被灼熱的火燄漩渦燒到皮開肉綻。\n"+NOR,enemy[i]);
     COMBAT_D->report_status(enemy[i], 1);
     enemy[i]->receive_damage("kee", 90);
     enemy[i]->apply_condition("burn",1);  
      enemy[i]->start_busy(1);
  COMBAT_D->report_status(enemy[i], 1);
 
}else{
      message_vision(HIR+"紅色的烈燄自火龍口中湧湧而出 ,正朝著$N迅速狂捲而去!!\n"+NOR,enemy[i]);
     message_vision(HIC+"只見$N飛身側閃 ,輕輕避開火龍的攻擊...\n"+NOR,enemy[i]);
