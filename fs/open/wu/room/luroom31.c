...
void create ()
{
  set ("short", "夢居");
  set ("long", @LONG
這是任正晴的寢室，擺設十分簡單，只有一張床和一些簡單家俱看
似單純，卻暗藏玄機，聽說任正晴為了怕人趁他睡覺偷襲他，暗藏
了許多機關，沒事別亂闖這裡，免的死於非命！
LONG);
  set("exits", ([ /* sizeof() == 1 */
...
int do_move(string str)
{
        if(!str || str != "桌子")
                ...
                message_vision("$N移動了木桌，此時赫然發現木桌的右上桌腳出現了一個暗扣(button)。\n", this_player());
        }
        else {
                delete_temp("box_moved");
                message_vision("$N將木桌移回原位。\n", this_player());
                if(query("exits/down"))
                        delete("exits/down");
        }
        return 1;
