...
void create()
{
  set_name("北冰洋神柱", ({"stela"}));
  set("long","調節北冰洋海域水量的古老神柱，據說擁有自我修復的奇妙能力，若是想強行破壞(punch)的話...。\n");
  set("unit","根");
  set_weight(1000000);
  set("value",10000);
  set("no_get",1);
...
int do_punch(string str)
{
  ...
  {
   if(!(present("ghost fish")))
   {
    message_vision(HIY"正當$N將要下手之際，守護"+room->query("short")+""HIY"的北冰洋海神出現制止了$N的動作。\n"NOR,me);
    mob = new("open/north-polar/npc/ghost-fish");
    mob->move(room);  
    mob->kill_ob(me);
   }else{
    mob = present("ghost fish");
