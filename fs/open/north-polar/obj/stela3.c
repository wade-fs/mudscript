...
void create()
{
  set_name("洞庭湖神柱", ({"stela"}));
  set("long","調節洞庭湖水域水量的古老神柱，據說擁有自我修復的奇妙能力，若是想強行破壞(punch)的話...。\n");
  set("unit","根");
  set_weight(1000000);
  set("no_get",1);
  set("no_sac",1);
...
int do_punch(string str)
{
  ...
  {
   if(!(present("saber-toothed tiger")))
   {
    message_vision(HIY + "正當$N將要下手之際，守護"+room->query("short")+"" + HIY + "的洞庭湖水神出現制止了$N的動作。\n" + NOR,me);
    mob = new("open/north-polar/npc/tiger");
    mob->move(room);  
    mob->kill_ob(me);
   }else{
    mob = present("saber-toothed tiger");
