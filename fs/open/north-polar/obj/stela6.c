...
{
  set_name("東海神柱", ({"stela"}));
  set("long","調節東海海域水量的古老神柱，據說擁有自我修復的奇妙能力，若是想強行破壞(punch)的話...。\n");
  set("unit","根");
  set_weight(1000000);
  set("no_get",1);
  set("no_sac",1);
...
int do_punch(string str)
{
  ...
  if(!room->query_temp("can_punch"))
  {
   if(!(present("sea girl")))
   {
    message_vision(HIY"正當$N將要下手之際，守護"+room->query("short")+""HIY"的東海海神出現制止了$N的動作。\n"NOR,me);
    mob = new("open/north-polar/npc/sea-girl");
    mob->move(room);  
    mob->kill_ob(me);
   }else{
    mob = present("sea girl");
