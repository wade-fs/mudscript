...
inherit NPC;
void create() {
set_name(CYN + "驌驦駒" + NOR, ({"sueswan horse" , "horse"}));
set("title", "戰馬");
set("long", "
牠是一匹在戰場上縱橫無阻，能夠助主人橫掃千軍的馬，這匹馬通
常是由元帥或將軍所騎，牠一身潔亮的白毛，與一張金色的馬鞍配
稱，看起來更為高貴。\n");
set("str",35);
set("cor",35);
set("cps",35);
...
int do_full() {
	...
	if(!me->query_temp("is_riding"))
        ...
        me->add("force", me->query("max_force")/24);
...
突然間，驌驦駒像發狂似的向$N衝撞而來\n"+
"$N閃避不及，被撞個四腳朝天，鮮血直吐\n" + NOR,target);
target->receive_damage("kee",100,mob);
      target->start_busy(1);
