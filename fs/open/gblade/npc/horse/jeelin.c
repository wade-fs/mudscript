...
inherit NPC;
void create() {
set_name(HIG + "騏驎駒" + NOR, ({"jeelin horse" , "horse"}));
set("title", "萬里馬");
set("long", "
牠是一匹能夠日行萬里且可三日不吃不喝的好馬，從牠光亮的的鬃毛
看來，的確是匹上等好馬，能夠擁有他的人，想必也是武林中赫赫有
名的人物。\n");
set("str",40);
set("cor",40);
set("cps",40);
...
if(me->query("family/family_name")=="魔刀門" ||
   me->query("family/family_name")=="魔刀莫測") {
        ...
        me->add_temp("apply/parry",25);
        me->add_temp("apply/defense",25);
        me->add_temp("apply/damage",25);
        me->add_temp("apply/armor", 25);
        me->add_temp("apply/dodge", 25);
        me->add_temp("apply/move", 50); } else {
        me->add_temp("apply/attack", 25);
        me->add_temp("apply/parry",25);
        me->add_temp("apply/defense",25);
        me->add_temp("apply/damage",25);
...
int do_full() {
	...
	if(!me->query_temp("is_riding"))
        ...
        me->add("force", me->query("max_force")/16);
