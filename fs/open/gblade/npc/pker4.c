...
void create()
{
        set_name("方  紀", ({"retainer"}));
        set("title", "蔡總管門下客之");
        set("nickname", "神龍鐵拳");
        set("long", "他是蔡總管所養的一位門下客。\n");
        set("age", 58);
        set("str", 30);
        set("force", 1000);
        set("max_force", 1000);
...
void init()
{
	...
	m_skill = query_skill_mapped(skill);
        ...
        mob->set_skill("umaarmed", lv_1);
        mob->set_skill("lungshan", lv_2);
        ...
        if( mob->query_temp("idle") == 3 ) {
            ...
            if( enemy = mob->query_temp("enemy") ) {
                if( environment(enemy) != environment(mob) &&
                    ...
                    tell_room(environment(enemy), name()+"走了過來。\n");
                    mob->kill_ob(enemy);
                    enemy->kill_ob(mob);
                }
            }
