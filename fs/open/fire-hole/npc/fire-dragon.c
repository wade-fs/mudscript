...
//修正if(total_exp > 22000) total_pot = 22000 判斷參數錯誤 by blazakira 2011/8/20
...
//                "\n火龍鱗耐火值為"+defire+",火龍原始害值為"+ohurt+",最後傷害值為"+hurt+"!!\n" + NOR,mob);                
  ...
  for(j=0 ; j < i ; j++)
  {
    ...
    {   
      if( enemy[j]->is_character() && !enemy[j]->is_corpse() && living(enemy[j]) &&
          enemy[j] != mob && environment(mob)==environment(enemy[j]) && userp(enemy[j]) && !enemy[j]->query("shan_yun_sword") )
      {
...
void die()
{
  ...
  {
    ring_type = winner->query_kar();
    if(winner->query_kar()>35)
      ring_type=36;         
    chance0 = sqrt(ring_type);
    chance = random(chance0)+random(4);
    if(userp(winner)) log_file("kill_dking", sprintf("%s(%s) 打死火龍王 on %s\n"
  ...
  for (j=0 ; j < i ; j++)
  {
    if( enemy[j] && environment(mob) == environment(enemy[j]) && userp(enemy[j]) && enemy[j]->is_character() && !enemy[j]->query("shan_yun_sword") )
    {
      CHANNEL_D->do_channel(this_object(),"sys",sprintf("%O,%d",enemy[j],enemy[j]->query_temp("fight_dking")));
  ...
  else if(h==4)
  {
    ...
    if(total_pot > 22000)  total_pot = 22000;
  }
  for (j=0 ; j < i ; j++)
  {
    if(enemy[j])
    {
      if(environment(this_object())==environment(enemy[j]))
      {
        if(k!=0)
        {
          exp = total_exp*(enemy[j]->query_temp("fight_dking")/10000) / (k/10000+1);
          pot = total_pot*(enemy[j]->query_temp("fight_dking")/10000) / (k/10000+1);
        } else {
...
//加入銀針門神農quest所需 temp by blazakira
  if (winner && winner->query("family/family_name")=="銀針門" && random(100) > 30  && query_temp("record_damage/"+winner->query("id")) > query("max_kee")/5) {
    winner->set_temp("spiritup","dragon king");
  }
...
void record_damage(int damage, object who)
{
  ...
  add_temp("record_damage/"+who->query("id") , damage); //開啟本紀錄並改為紀錄在npc身上作為銀針quest用 //下方判斷為參與火龍王戰鬥的經驗分享判斷 由於會在上方的判斷中刪除 因此暫不使用相同紀錄 避免影響
  who->add_temp("fight_dking",damage);
}
