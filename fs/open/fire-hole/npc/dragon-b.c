...
{
  object ob;
  set_name("藍光火龍", ({ "blue dragon","dragon","blue" }) );
  set("long",
    "全身泛著藍光的火龍，守護著火龍窟的結界交點。\n");
  set("race", "野獸");
  set("title",HIC + "(藍光)" + NOR);
  set("age", 50000);
...
*/
  ...
  if (mob->query("force") < 1000000) { // 總要讓 dragon 用什麼來轉換吧？wade
    ...
    message_vision(HIR + "\n藍光火龍燃起熾熱火焰將自身的內力倍增六成!!\n" + NOR,mob);
  }

  if ( mob->query("kee")<130000 && random(200)<50 && !mob->is_busy())
  {
/*
    mob->receive_curing("kee",4000);
    mob->receive_heal("kee",4000);
*/
    message_vision(HIY + "\n藍光火龍運起神功\為自身療傷，傷口也慢慢癒合了\n" + NOR,mob);
        ...
        if(enemy[j]){
              ...
              environment(this_object())==environment(enemy[j]))
          {
            ...
            if(!enemy[j]->query("ring_type"))
              ...
              defire=2000;
            if (enemy[j]->query("family/family_name")=="雪蒼派" && enemy[j]->query_temp("keeup")==1)
            {
              hurt = 3000+random(2000);
              enemy[j]->apply_condition("hart",100);
              enemy[j]->set("force_factor",0);
              tell_object(enemy[j],HIY + "由於你的雪蒼內勁被火焰打散, 使你無法施力並受到數倍的傷害!!\n" + NOR);
            }
...
void die()
{
  ...
  for (j=0 ; j < i ; j++)
  {
    ...
    {
      k = k + enemy[j]->query_temp("fight_blue");
      h = h + 1;
    }
  }
  if(h==1)
  ...
  for (j=0 ; j < i ; j++)
  {
    if(enemy[j]){
      if(environment(this_object())==environment(enemy[j]))
      {
        if(k!=0)
        {
          exp = total_exp*(enemy[j]->query_temp("fight_blue")) / k;
          pot = total_pot*(enemy[j]->query_temp("fight_blue")) / k;
        }else{
          exp = 9000;
          pot = 1800;
        }
  ...
  if( present("dragon claws",this_object()) )
  {
    ...
    environment(ob)->set_temp("ko_b",1);
    destruct(ob);
  }
