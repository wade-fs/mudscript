...
string m1="$N手中長劍虹芒大盛，「劍影連陽」，往$n直刺而去。\n\n";
void array1();
void drunk();
...
//16
            ([  "action"     :                "\n$N低吟道：
               『斬風截雪破千秋               初冬舞塵一朝仇 
                 笑任青天縱豪情               劍狂情狂人更狂』
吟罷舞劍而起，使出了仙劍派的殺招『碧波殺機映』！
招如其名，在$N長劍舞動下碧綠劍氣縱橫宛如陣陣波濤，卻掩不住暗藏其中的殺意",
                "dodge"      :                60,
                "parry"      :                50,
                "damage"     :                320,
                "force"      :                320,
...
//17                      
            ([  "action"     :                "\n$N回劍入鞘，緩緩說道：
              『一蕭一劍平生意               負盡狂名殺無情』
說罷眼中殺意陡盛，拔劍出鞘，使出了仙劍派禁招『落日殘情滅』！
$N迅速地揮動手中的$w，只見滿天血紅宛若落日的餘暉，卻原來是$n被殺招所傷，滿身鮮血飛揚於空中！",
                "dodge"      :                60,
                "parry"      :                50,
                "damage"     :                340,
                "force"      :                340,
...
void sp_att2(object me, object victim, object  weapon, int damage)
 {
    ...
    { 
     message_vision("寒箏劍劍勢迥異難測 ,加上第三招之變幻莫測 ,$n一個不留神中了$N 一記強擊且暫時無法對$N展開攻擊\n",me,victim);
    victim->add("kee",-400);    
    victim->start_busy(1);
    COMBAT_D->report_status(victim, 1);
    }
 ...
 int skill_level=me->query_skill("shasword",1);
  if( random(100) > 70 && me->query("force",1) > 100)
  {
    if(!me->query_temp("conti"))
    {
     ...
     if( me->query("family/master_name")=="張乘風" || 
     ...
     if( me->query("family/master_name")=="張乘雲" ||
