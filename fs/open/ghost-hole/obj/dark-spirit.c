...
void create()
{
  set_name("心魔闇之魂",({"dark-spirit"}));
  set("long","若有似無，黑色半透明的魂靈物體，夾帶著一股攝人心魄的神秘力\n"
...
int hurt()
{
  ...
  me=environment(ob);
  if( !ob || !me ) return 1;
  ob1=present("dark-emblem",me);
  ob2=present("dark-spirit",me);
  ...
  if( ob2 && userp(me) && !me->is_busy() )
  {
    if( ob1 )
    {
      if( random(7) == 1 )
      {
        ...
        me->add("force",-30);
        message_vision("$n" + BLU + "緩緩散開一股黑色的氣霧，卻被"NOR+ob1->query("name")+BLU+"緩緩的吸收一部份！\n" + NOR,me,ob);
      }
    } else {
      if( random(2) == 1 )
      {
        ...
        me->add("force",-50);
        message_vision("$n" + BLU + "緩緩散開一股黑色的氣霧，慢慢的吸蝕著" + HIY + "$N"NOR+BLU"的全身精元！\n" + NOR,me,ob);
      }
    }
  }
  call_out("hurt",3,ob);
