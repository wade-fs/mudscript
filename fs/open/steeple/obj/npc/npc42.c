...
if(random(9) >= 5)
{
message_vision(HIY + "瀧山武者拳掌運火，往你身上一拍，頓時烈火纏身！\n" + NOR,me);
for(j=0;j < i;j++) {
target[j]->add("kee",-1900); }
   }
   else
   {
message_vision(HIC + "瀧山武者仰天一笑，瞬時內息穩固。\n" + NOR,me);
if(random(9) >= 5) {
...
if(me->query("quest/start_game")< 43)
       {
        message_vision(HIY + "
恭喜"HIW+me->query("name")+HIY"挑戰" + HIG + "『" + HIR + "第四十二層試煉ソ塔" + HIG + "』" + HIY + "通過！！

" + HIW + "希望"+me->query("name")+"能不負眾望，勇闖" + HIG + "『" + HIY + "第四十三層試煉ソ塔" + HIG + "』" + HIW + "。
	" + NOR,me);
