//by frequency
//修正訊息顏色的錯誤 並補上少林後山任務的線索 by blazakira 2010/10/22
#include <ansi.h>
inherit NPC;
string ask_old_man();
string ask_swordsman();

string ask_theif();
void create()
{
        set_name("薛農",({"information spy","spy"}));
        set("long",@LONG
...
LONG);
        ...
        set("inquiry",([
              "太極長老": (:ask_old_man:),
              "swordsman": (:ask_swordsman:),
              "神秘飛賊" : (: ask_theif:)
              ]));
          set("attribute","wind");
          set("dev_obj/wind",9999999);
              set("weapon/wind-full",999);
        setup();

        carry_object("/autoload/open-area/super_hands")->wear();
        carry_object("/autoload/north-polar/light-ring");
        carry_object("/autoload/open-area/plate");
        carry_object("/autoload/open-area/light_cloak");
        carry_object("/autoload/open-area/armband");
        carry_object("/autoload/open-area/turtle-shield");
//          carry_object("/autoload/sky/night-legging");
        carry_object("/autoload/sky/peace_neck");
        carry_object("/autoload/open-area/divine_belt");
        carry_object("/open/dancer/obj/linrboots");
        carry_object("/autoload/open-area/five-turn");
        carry_object("/autoload/open-area/snake_amulet");
          carry_object("/open/sky/obj/claw");
}
string ask_old_man()
{
  if(!this_player()->pay_money(10000))
...
string ask_theif()
{
  if(!this_player()->pay_money(10000000))
...
int accept_object(object who, object ob)
{
        int value=ob->value();
        string str;
        if(ob->query("id") != "coin" && ob->query("id") != "silver" && ob->query("id") != "gold" && ob->query("id") != "cash" && ob->query("id") != "diamond")
          {
           command("say 老子要的是錢啊，錢錢錢~~ 不是這些廢物！");
           write("薛農變出一個黑洞，將"+ob->query("name")+"吞沒。\n");
           destruct(ob);
           return 1;	
          }
         ...
         switch(random(18))
                  {
                  	case 0:
                        ...
                        break;
                        case 1:
                        str = "據說每天早晚10:00左右都會有神秘劍客出沒，打敗他們得到的戰功\，可以在京城換取防具喔！\n";
                        break;
                        case 2:
                        ...
                        break;
                        case 17:
                        str = "聽說周芷若在尋找一對刀劍，使用正確方式交給她之後便能獲得相對應的任務喔。\n";
                        break;
                  }
                command("say "+str);
