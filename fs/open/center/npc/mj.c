...
void create()
{
set_name("深田恭子",({"mj", "mj's server"}));
        set("long",@LONG
  深田恭子(Mj 小姐)，她專門看台子的，有問題請問她：
        麻將，玩法，規則
LONG);
        set_weight(1);
        set("unit","張");
...
int do_dump(string str,object me)
{
        ...
        if (count==(end-16-Goned)){
           ...
           str="所剩下的牌隻有:"+show_mj(List[count..end],2)+"\n";
...
/////////////
        ...
        for (i=0; i<26; i++) Mj+="█";
        Mj+="◤\n";
        Mj+="█▏    "+HIW+"中文名字  已玩局數  總戰蹟  自摸次數  放槍次數▏\n";
        for (i=0;i<Play;i++){
        ...
        if (arg=="1p"||arg=="2p"||arg=="3p"||arg=="4p"){
           ...
           Mj+="你的設定有:";
              ...
              Mj+=show_mj(P4Data["Dump"],2)+"\n";
                 ...
                 if (Playing){
                    Mj=LookP+"正認真的在打牌\n";
                    Mj+="還剩下"+sprintf("%d",(end-count-16-Goned)/2)+"隻牌就流局了。\n";
           ...
           Mj+="還有"+chinese_number((end-count-16-Goned)/2)+"隻牌就流局了\n";
        }else if (arg=="dump"){
...
int show_top(string str)
{
        ...
        for (i=0; i<34; i++) show+="　";
        show+=HBWHT+"◢\n";
        show+="█"+HBRED+"▏ 英文名字 │  中文名字  │已玩局數│ 總戰蹟 │自摸次數│放槍次數▕"+HBWHT+"█\n";
        if (str=="all"){
