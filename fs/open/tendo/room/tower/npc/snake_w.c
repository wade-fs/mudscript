...
int i=random(2)+1,j=random(2)+1,k=random(2)+1;
...
void die()
{
  ...
  if(winner)
  {
    ...
    message_vision(HIY"\n"+winner->name()+"打死"+this_object()->query("name")+"得到二十五點戰功\!!\n"NOR,winner);
    write_file("/log/get_war/snake",sprintf("%s(%s)    打死%s得到二十五點戰功\於 %s\n",winner->name(1),winner->query("id"),this_object()->query("name"),ctime(time())));
    winner->add("war_score",25);
  }
  :: die();
}
