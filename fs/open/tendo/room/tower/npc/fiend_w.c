//增加移動的機會 與補上攻擊方參數 by blazakira 2013/9/8
//增加死亡時 last_damage_from 的判定 by blazakira 2013/9/19

#include <ansi.h>
inherit NPC;

int i=random(9)+1,j=random(9)+1,k=random(9)+1;
...
void create()
{
  ...
  set("long","與天道派有著極深的仇恨，曾大鬧人間數十年，而遭到天道派的禁錮。\n");
...
void heart_beat()
{
  ...
  if( n >= 2 && !me->query("help") )
  {
    command("say 想找幫手，是得不到你想要的東西的！！");
    if( present("book",me) )
    {
      destruct(present("book",me));
      message_vision(HIR + "只聽到「轟」的一聲，「天書」已成碎屑滿天飄散了。\n" + NOR,me);
      me->set("help",1);
    }
  }
...
" + HIW + "$N屈體弓身，雙掌緊貼地面，全身籠罩在" + HIW + "一片藍光中，地面宛如波濤般浮動，隨著蓄積的內勁，藍光不斷" + HIW + "增加，只見$N突然將雙掌一合，一道駭人藍箭向$n疾射而去!\n" + NOR,me,enemy[j]);
            enemy[j]->receive_damage("gin",enemy[j]->query("max_gin")/13,me);
            enemy[j]->receive_damage("sen",enemy[j]->query("max_sen")/13,me);
            enemy[j]->receive_wound("kee",enemy[j]->query("max_kee")/13,me);
            COMBAT_D->report_status(enemy[j]);
            break;
...
" + HIW + "$N以氣貫入雙爪 ,雙爪閃出一陣耀目金光," + HIW + "氣提十二成 ,勁注全身 ,蓄勢準備使出最強最霸道的一式,霎時" + HIW + "狂龍魔氣急速旋繞$N ,無儔魔氣因此驚動天地之氣 ,互抗暴出" + HIW + "數道紫電雷光 ,$N忽然現身於$n前 ,對$n使出了石破天驚的一擊!\n" + NOR,me,enemy[j]);
            for(j=0;j<sizeof(enemy);j++)
            {
              ...
              if(environment(me) == environment(enemy[j]))
              {
                enemy[j]->receive_damage("gin",enemy[j]->query("max_gin")/11,me);
                enemy[j]->receive_damage("sen",enemy[j]->query("max_sen")/11,me);
                enemy[j]->receive_wound("kee",enemy[j]->query("max_kee")/11,me);
                COMBAT_D->report_status(enemy[j]);
              }
            }
            break;
...
" + HIW + "無窮無盡的勁力由四周湧入$N體內，只見冥" + HIW + "殿修羅雙掌不停做著細微的變化，彷彿如黑洞般，將萬物都吞食" + HIW + "淹滅,忽然雙掌擊向$n的全身!\n" + NOR,me,enemy[j]);
            for(j=0;j<sizeof(enemy);j++)
            {
              ...
              if(environment(me) == environment(enemy[j]))
              {
                enemy[j]->receive_damage("gin",enemy[j]->query("max_gin")/9,me);
                enemy[j]->receive_damage("sen",enemy[j]->query("max_sen")/9,me);
                enemy[j]->receive_wound("kee",enemy[j]->query("max_kee")/9,me);
                COMBAT_D->report_status(enemy[j]);
              }
            }
            break;
...
" + HIW + "$N狂吼一聲,上空平身挪後三寸,左手拍地,右" + HIW + "手股動內勁,破天而出,掌勁如山,激起無濤的氣勁,夾著雷霆萬駒" + HIW + "之勢朝$n而來!!\n" + NOR,me,enemy[j]);
            for(j=0;j<sizeof(enemy);j++)
            {
              ...
              if(environment(me) == environment(enemy[j]))
              {
                enemy[j]->receive_damage("gin",enemy[j]->query("max_gin")/7,me);
                enemy[j]->receive_damage("sen",enemy[j]->query("max_sen")/7,me);
                enemy[j]->receive_wound("kee",enemy[j]->query("max_kee")/7,me);
                COMBAT_D->report_status(enemy[j]);
              }
            }
            break;
...
" + HIW + "$N左足橫移,挫腰沈身,單袖一揚,瞬時狂風拂" + HIW + "起, 塵沙飛揚,$N趁飛沙走石之際,運起陽剛真氣,身形一旋," + HIW + "雙掌內攏,催起一股凌厲的氣焰,朝$n的天靈蓋\連續拍出數掌!\n" + NOR,me,enemy[j]);
            for(j=0;j<sizeof(enemy);j++)
            {
              ...
              if(environment(me) == environment(enemy[j]))
              {
                enemy[j]->receive_damage("gin",enemy[j]->query("max_gin")/5,me);
                enemy[j]->receive_damage("sen",enemy[j]->query("max_sen")/5,me);
                enemy[j]->receive_wound("kee",enemy[j]->query("max_kee")/5,me);
                COMBAT_D->report_status(enemy[j]);
              }
            }
            break;
...
" + HIW + "$N閃身反掌擊出，一道青色光芒從掌中發出，" + HIW + "使$n分不清身在何方，只見$N身形一展，幻化成千萬身影," + HIW + "身影中夾帶著沉重的掌勁向$n罩下,$n正在遲疑之際已被無數掌影所擊中!\n" + NOR,me,enemy[j]);
            for(j=0;j<sizeof(enemy);j++)
            {
              ...
              if(environment(me) == environment(enemy[j]))
              {
                enemy[j]->receive_damage("gin",enemy[j]->query("max_gin")/3,me);
                enemy[j]->receive_damage("sen",enemy[j]->query("max_sen")/3,me);
                enemy[j]->receive_wound("kee",enemy[j]->query("max_kee")/3,me);
                COMBAT_D->report_status(enemy[j]);
              }
            }
            break;
...
" + HIW + "$N重重的掌,連疊的拳,萬鈞之力,渾雄之氣,煞" + HIW + "時天空彷彿突然黑暗下來,妖魔彷彿都從墓地爬了起來,推山倒海," + HIW + "大地翻滾,縱橫上下空間,為掌風與掌影所佈,有如綿綿飛際的利刃擊向$n!!\n"+NOR,me,enemy[j]);
            for(j=0;j<sizeof(enemy);j++)
            {
              ...
              if(environment(me) == environment(enemy[j]))
              {
                enemy[j]->receive_damage("gin",enemy[j]->query("max_gin")/2);
                enemy[j]->receive_damage("sen",enemy[j]->query("max_sen")/2);
                enemy[j]->receive_wound("kee",enemy[j]->query("max_kee")/2);
                COMBAT_D->report_status(enemy[j]);
              }
            }
            break;
  ...
  if( !me->is_fighting() ) 
  {
    ...
    if(random(10) == 0) //增加很會跑的判斷 by blazakira 2013/9/8
      random_move();
  }
  ...
  if(winner)
  {
    if( winner->query("class") == "taoist" )
    {
      if( !me->query("help") && !winner->query("quests/god_book") )
      {
        ...
        {
          destruct(present("book",me));
          new("/open/tendo/room/tower/obj/book_true")->move(environment(winner));
          message_vision(HIY + "一陣光芒之後「天書」由冥殿修羅的身體中浮現空中。" + NOR,me);
          winner->set("quests/god_book",1);
        }
      }
    }
    tell_object(users(),HIB + "\n\n只見天空烏雲消散，又恢復了一片祥和。\n\n" + NOR);
    message_vision(HIY + "\n"+winner->name()+"打死"+this_object()->query("name")+"得到六十點戰功\!!\n" + NOR,winner);
    write_file("/log/get_war/fiend",sprintf("%s(%s)    打死%s得到六十點戰功\於 %s\n",winner->name(1),winner->query("id"),this_object()->query("name"),ctime(time())));
    winner->add("war_score",60);
  }
  :: die();
}
