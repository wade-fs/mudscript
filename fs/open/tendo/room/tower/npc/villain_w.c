...
int i=random(5)+1,j=random(5)+1,k=random(5)+1;

void create()
{
  set("title","冥殿第五殿殿主");
  set_name("血面羅剎",({"villain"}));
  set("gender","男性");
  set("age",700);
...
void die()
{
  ...
  if(winner)
  {
    ...
    message_vision(HIY"\n"+winner->name()+"打死"+this_object()->query("name")+"得到四十點戰功\!!\n"NOR,winner);
    write_file("/log/get_war/villain",sprintf("%s(%s)    打死%s得到四十點戰功\於 %s\n",winner->name(1),winner->query("id"),this_object()->query("name"),ctime(time())));
    winner->add("war_score",40);
  }
  :: die();
}
