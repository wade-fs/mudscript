...
int i=random(8)+1,j=random(8)+1,k=random(8)+1;

void create()
{
  set("title","冥殿第八殿殿主");
  set_name("生死一屠",({"butcher"}));
  set("gender","男性");
  set("age",800);
...
void die()
{
  ...
  if(winner)
  {
    ...
    message_vision(HIY + "\n"+winner->name()+"打死"+this_object()->query("name")+"得到五十五點戰功\!!\n" + NOR,winner);
    write_file("/log/get_war/butcher",sprintf("%s(%s)    打死%s得到五十五點戰功\於 %s\n",winner->name(1),winner->query("id"),this_object()->query("name"),ctime(time())));
    winner->add("war_score",55);
  }
  :: die();
}
