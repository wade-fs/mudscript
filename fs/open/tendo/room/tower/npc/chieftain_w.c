...
int i=random(6)+1,j=random(6)+1,k=random(6)+1;

void create()
{
  set("title","冥殿第六殿殿主");
  set_name("獠牙文魁",({"chieftain"}));
  set("gender","男性");
  set("age",720);
...
void die()
{
  ...
  if(winner)
  {
    ...
    message_vision(HIY"\n"+winner->name()+"打死"+this_object()->query("name")+"得到四十五點戰功\!!\n"NOR,winner);
    write_file("/log/get_war/chieftain",sprintf("%s(%s)    打死%s得到四十五點戰功\於 %s\n",winner->name(1),winner->query("id"),this_object()->query("name"),ctime(time())));
    winner->add("war_score",45);
  }
  :: die();
}
