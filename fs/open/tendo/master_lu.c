...
於此加以保護，以防節外生枝壞了祖師爺五百年前的一番浴血苦戰。" + NOR + "");
  ...
  return (
"此陣殺惡不殺善，是專防妖魔侵入而設。
如殺氣過高 陣中雲龍會誤以為妖魔來襲而攻擊。

接著陸清風語帶雙關地說道 : 此陣更分為正逆兩儀，由八卦衍
生而出，正行可到天道，逆行可致魔妖，切記切記。");
}
string do_tenman()
{
if (this_player()->query("quests/shasword")==2)
{
command("say 天門奇陣乃利用開天三靈器,融合三聖之力所形成的一個傳說奇陣!");
}
else command("say 聽都沒聽過");
}
string do_howgo()
{
 object who=this_player();
  if(who->query("class")!="taoist")
        return (
"此事乃本門機密，"+RANK_D->query_respect(who)+"不必多問了。");
  return (
"這鎖妖塔的妖怪厲害異常，就算是當年，祖師爺也費了九牛二虎之力才將其封住。
 其中有一隻強力妖怪由鎮塔明王守著!!");
}
