...
void heart_beat()
{
	...
	if( random(100) < 5 && is_fighting() ) { // 主打 busy
	...
	if( random(100) < 10 && is_fighting() ) { // 主打 force
		message_vision( HIR"
	...
	if( random(100) < 5 && is_fighting() ) { // 主打神
		message_vision( HIR"
	...
	if( random(100) < 5 && is_fighting() ) { // 主打mob
		message_vision( HIR"
	...
	if( random(100) < 10 && is_fighting() ) { // 主打氣
		message_vision( HIR"
	焚天魔王化身煉獄業火, 妖氣在一瞬間變成沸騰血紅魔風,
	凡被其吹拂之物盡揮成汽, 熔人口鼻令人窒息
	眨眼間, 焚天魔王叱喝迴盪"HIW"『赤獄混沅 魔道遮天』"HIR", 熔融魔氣挾著蝕骨
...
"NOR);
 ...
 log_file("mogi/KILL_FIRE_KING", sprintf("%s(%s) 打敗 焚天魔王 on %s\n"
