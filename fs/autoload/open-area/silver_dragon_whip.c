//silver_dragon_whip.c by frequency
#include <ansi.h>
#include <weapon.h>
inherit WHIP;
...
void create()
{
        ...
        if( clonep() )
                ...
                set("long",
HIW + "閃爍著耀眼銀光的魔鞭，是傳說中八岐大蛇後裔歷經千年修道而成的肉身。
請以fusion、dream、star、defense使用相關特技\n" + NOR);
        }
        init_whip(70);
...
int do_fusion()
{
    if(!query("equipped")) return notify_fail("先裝備上再說吧!!\n");
    else{
...
void heart_beat()
{
   if(!me || me->query_temp("unconcious"))
  {
   set_heart_beat(0);
   return;
  }
    if(me->query("family/family_name")=="冥蠱魔教" || me->query("family/family_name")=="天道派")
    {
    if(query("equipped") && me->is_busy() && random(110) < kar+spi+per) 
     {
...
if(70>random(100)) {
  ...
  HIY + "$n完全被眼前奇景迷惑住，警戒鬆懈、毫不設防。\n" + NOR,me,target[j]);
            ...
            {
            target[j]->start_busy(random(busy)+1);
...
message_vision(HIR + "
  ...
  慢慢在空氣中擴散，$n冷不防吸了一大口...
" + NOR,me,target[j]);
                target[j]->apply_condition("evil_poison",random(5)+5);
                target[j]->apply_condition("evil_pill01",random(5)+5);
                target[j]->apply_condition("evil_pill02",random(5)+5);
COMBAT_D->report_status(target[j]);
               }
        }
        me->start_busy(1);
...
HIC + "                     無數隕石降下，挾帶著星之力如雨點般落於$n肉身，
                   ...
                   if (userp(me))
                   {
                      ...
                      target[j]->receive_damage("gin",gdamage/10);
                      target[j]->receive_damage("sen",sdamage/10);
                      target[j]->receive_damage("kee",kdamage/10);
                      target[j]->start_busy(random(3)+1);
                        }
              COMBAT_D->report_status(target[j]);
            }
...
HIR + "                     光芒籠罩的中心點，$n三魂七魄直接遭受星辰靈氣衝擊，
                            內息不知不覺間受到牽引，頓時一陣紊亂......
                     \n" + NOR,me,target[j]);
                 target[j]->apply_condition("no_power_e",random(5)+5);
                 target[j]->apply_condition("no_power_f",random(5)+5);
                 target[j]->apply_condition("tsunami",random(5)+5);
                 target[j]->start_busy(random(3)+1);
                }
        }
        me->start_busy(1);
...
int do_perform4()
{
   if(me->query("family/family_name")!="冥蠱魔教" && me->query("family/family_name")!="天道派")
   return notify_fail("......\n");
   if(!me->query_temp("chain-array")) 
   return notify_fail("你先前並未解放龍的力量。\n");
   if(me->query("family/family_name")=="冥蠱魔教")
    {
    	(int)me->add_temp("apply/dodge",-funp);
    	(int)me->add_temp("apply/move",-funp);
