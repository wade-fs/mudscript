...
//改進使用者的判斷 並補上receive_xxx的攻擊方之參數 by blazakira 2011/4/29
...
int do_compose(string arg)
{
  ...
  if( arg == "ball" )
  {
    if( !present("five turn ball",me) )
      return notify_fail("你在身上翻來翻去，卻怎樣也找不出這樣東西!!\n");

    if(!ob->query("five-ball")==1 && (present("five turn ball",me)->query("five-ball")==1) )
    {
      message_vision(HIM"\n$N"HIM"將"HIC"五轉"HIW"聚"HIY"靈珠"HIM"輕輕的裝在$n"HIM"水晶球凹槽上，\n$N"HIM"見到$n"HIM"發出更強大的靈光快速的運轉!!\n"NOR,me,ob);
      ob->set("five-ball",1);
      me->set_temp("five-ball",6);
...
*/
  ...
  message_vision(HIY"只見$n"HIY"內蘊含的魂魄靈力開始運轉不停，發出柔和的光芒。\n"NOR,me,ob);
...
*/
  ...
  message_vision("$n"NOR+YEL"內蘊含的魂魄靈力慢慢停止運轉，柔和的光芒漸漸暗淡。\n"NOR,me,ob);
...
//態，單一出現機率也很小，還有另一層條件滿足，每一種的不同，所以實
//際出現機率更小
      if( su1 == 1 && random(100) < (6+fv) )//金色主定人
      {
      ...
      if( su2 == 1 && random(100) < (6+fv) )//綠色主回復
      {
      ...
      if( su3 == 1 && random(100) < (6+fv) )//藍色主化內力為己用和降enforce
      {
      ...
      if( su4 == 1 && random(100) < (6+fv) )//紅色主攻精氣神
      {
          ...
          if(!enemy[j]) return ;
          {
            if( environment(me) == environment(enemy[j]) )
            {
              if( k > 0 )
              {
                ...
                enemy[j]->receive_damage("kee",random(500)+1000,me);
                enemy[j]->receive_damage("gin",random(250)+500,me);
                enemy[j]->receive_damage("sen",random(250)+500,me);
      ...
      if( su5 == 1 && random(100) < (6+fv) )//藍色主損精氣神之最大值
      {
          ...
          if(!enemy[j]) return ;
          {
            if( environment(me) == environment(enemy[j]) )
            {
              if( k > 0 )
              {
                ...
                enemy[j]->receive_wound("kee",1000,me);
                enemy[j]->receive_wound("gin",500,me);
                enemy[j]->receive_wound("sen",500,me);
                COMBAT_D->report_status(enemy[j]);
              } else {
