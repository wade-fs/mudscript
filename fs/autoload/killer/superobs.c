...
set_name(HIM + "魔霸封神指" + NOR,({ "Yuki-Fighter-kill","yuki"}) );
   set_weight(3500);
   if( clonep() )
   ...
   {
     set("long",@LONG
這乃吸收天地日月的精華而醞藏在凌雲山中的上古神器。此乃是 Yukigirl 所擁有，
這是她費盡千辛萬苦才得到的，而且這兵器似乎也能感受到她的靈性與智慧
所以已成了她的專屬武器。

     下列是你可以對他下的命令，詳細內容請打help yuki_fighter_kill
     鍛鍊(discipline)      格式 discipline           ：以自身力量使光系力量成長
     吸收(absorb)     	格式 absorb <target>  ：吸收對方使暗黑系力量成長
     命名(rename)   	格式 rename <name> ：為武器改名
     資料(dag_data)       格式 yuki_data           ：察看武器目前的水準
LONG);
     set("unit","對");
     set("value",0);
     set("material", "steel");
     set("wield_msg", HIR + "$N仰天狂笑裝備起$n，煞時紅光四射強大的肅殺之氣壟罩在四周\n" + NOR);
     set("unwield_msg", HIR + "$N收起殺意慢慢的將$n脫下，周圍的氣勁也隨之消減\n" + NOR);
     set("no_sell",1);
     set("no_drop",1);
     set("no_auc",1);
   }
...
void init()
{
   ...
   {
    if(this_player()->query("id") == "superobs"  || this_player()->query("id") =="wataru")
    {
       add_action("feed_dag","discipline");
       add_action("target_absorb","absorb");
       add_action("rename_dag","rename");
       add_action("verify_pet","yuki_data");
...
int target_absorb(string str)
{
	...
	message_vision(HIM + "$N揚起"+name()+HIM + "殺氣凝聚於"+name()+"幻出紫氣\n" + NOR,me);
...
int rename_dag(string arg)
{
   ...
   arg = trans_color(arg);
   this_object()->set_name(arg + NOR,({"Yuki-Fighter-Kill","yuki"}) );
   write("你替它取了個名字叫"+this_object()->name()+"。\n");
   return 1;
}
...
{//hit_ob的BUG，第一個傳入物件代表被害者，第二個不清楚
  ...
  if( user->is_fighting() && query("equipped") )
  {
	...
	{//為了擴展性，例如還可以加入火之屬性與水之屬性
	 ...
	 case 2:
	 {
    	  message_vision(HBMAG + "被戰鬥的氣勁牽引突然光芒一現,幻化成九龍凌空於身！" + NOR,user);
      ...
      message_vision(HIW + "$N巧妙的躲開"+this_object()->name()+"的攻擊\n" + NOR,victim);
  }
}

int help_dream(string str)
{
   if(str != "yuki_fighter_kill")  
         	return notify_fail("打錯了");
write(@HELP
魔霸封神指可以隨著你養育他的方法而成長，具有光明與黑暗兩系的力量。
兩種力量並不相容，所以當其中一方的力量增強，另一方的力量必然下降。
所以，想讓魔霸封神指同時具有兩種屬性是不可能的。
魔霸封神指在光系的力量是以回復自身為主，暗系的力量是以傷害對手為主。
因此，光系的力量必須以自身的內力去培養，
...
int query_autoload()
{
    if (this_player()->query("id") == "yukigirl")
        return 1;
}
