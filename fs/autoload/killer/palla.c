...
void create()
{
   ...
   {
     set("long",@LONG
當你眼睛被此把無名所吸引，殺氣頓時從刀鋒射出,令你無法直視此把無名
由無名所傳來的無窮威力，似乎只有揚能夠作他的主人
     下列是你可以對他下的命令，詳細內容請打help yang
     鍛鍊(discipline)      格式 discipline           ：以自身力量使光系力量成長
     吸收(absorb)     	格式 absorb <target>  ：吸收對方使暗黑系力量成長
     命名(rename)   	格式 rename <name> ：為武器改名
     資料(dag_data)       格式 yang_data          ：察看武器目前的水準
LONG);
     set("unit","把");
     set("value",0);
     set("material", "steel");
     set("wield_msg", HIB + "$N仰天狂笑裝備起$n，煞時紅光四射強大的肅殺之氣壟罩在四周\n" + NOR);
     set("unwield_msg", HIB + "$N收起殺意慢慢的將$n脫下，周圍的氣勁也隨之消減\n" + NOR);
     set("no_sell",1);
     set("no_drop",1);
     set("no_auc",1);
   }
...
int rename_dag(string arg)
{
   ...
   arg = trans_color(arg);
   this_object()->set_name(arg + NOR,({"yang"}) );
...
write(@HELP
無名可以隨著你養育他的方法而成長，具有光明與黑暗兩系的力量。
兩種力量並不相容，所以當其中一方的力量增強，另一方的力量必然下降。
所以，想讓無名同時具有兩種屬性是不可能的。
無名在光系的力量是以回復自身為主，暗系的力量是以傷害對手為主。
因此，光系的力量必須以自身的內力去培養，
