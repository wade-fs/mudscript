...
{
   seteuid(getuid());
   set_name(HIM + "狂傲魔風" + NOR,({ "reko sword","sword"}) );
   set_weight(5000);
   if( clonep() )
   ...
   {
     set("long",@LONG
    一把擁有神奇力量的劍看著那忽隱忽現的光芒,似乎透著Reko過去的一段傷心往事
   一股魔風襲擊而至,你的心中為Reko傷心無比.隱隱約約看到了幾個字似乎是.......
     ...我...愛...你...    詳細內容請打help reko_sword
     鍛鍊(discipline)        格式 discipline             ：以自身力量使光系力量成長
     吸收(absorb)     	  格式 absorb <target>    ：吸收對方使暗黑系力量成長
     命名(rename)   	  格式 rename <name>   ：為狂傲魔風改名
     資料(reko_data)         格式 reko_data              ：察看狂傲魔風目前的水準
LONG);
     set("unit","把");
     set("value",0);
     set("material", "steel");
...
{
   if(!arg) return notify_fail("你想為狂傲魔風取什麼名字呢?\n");
   arg = trans_color(arg);
   this_object()->set_name(arg + NOR,({ "star unarmed","star"}) );
   write("你替狂傲魔風取了個名字叫"+this_object()->name()+"。\n");
   return 1;
}
	...
	{//為了擴展性，例如還可以加入火之屬性與水之屬性
	 ...
	 {//目前是試作版，只有一些訊息與補氣
	  ...
	  //或許連武器上都可以做自己決定要用的特攻魔法
      		message_vision(HBCYN + "魔傲狂風上化出聖光氣,籠罩住$N!!" + NOR,victim);
		message_vision(HBCYN + "魔傲狂風由$N身上，回到$n的體內!!" + NOR,victim,user);
	 ...
	 case 2:
	 {
      		message_vision(HBMAG + "魔傲狂風上化出數道鬼影，衝向$N!!！！" + NOR,victim);
		message_vision(HBMAG + "$N在精神層面上受到很大的打擊!!" + NOR,victim);
      ...
      message_vision(HIW + "$N巧妙的躲開，魔傲狂風的攻擊\n" + NOR,victim);
  }
}
...
write(@HELP
狂傲魔風，可以隨著你養育他的方法而成長，具有光明與黑暗兩系的力量。
兩種力量並不相容，所以當其中一方的力量增強，另一方的力量必然下降。
所以，想讓狂傲魔風同時具有兩種屬性是不可能的。
狂傲魔風在光系的力量是以回復自身為主，暗系的力量是以傷害對手為主。
因此，光系的力量必須以自身的內力去培養，
...
int query_autoload()
{
    if (environment() && environment()->query("id") == "reko")
        return 1;
}
