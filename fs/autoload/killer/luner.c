...
void create()
{
   ...
   {
     set("long",@LONG
此把天恨之上，充滿著無盡的怨念，自古無人能馴，直到遇到真主-luner後
經過一番苦戰之後才為他所用，成為其專屬武器。

     下列是你可以對他下的命令，詳細內容請打help skyhate
     鍛鍊(discipline)      格式 discipline           ：以自身力量使光系力量成長
     吸收(absorb)     	格式 absorb <target>  ：吸收對方使暗黑系力量成長
     命名(rename)   	格式 rename <name> ：為武器改名
     資料(dag_data)       格式 skyhate_data           ：察看武器目前的水準
LONG);
     set("unit","對");
     set("value",0);
     set("material", "steel");
     set("wield_msg", HIR + "$N臉露恨意暴喝一聲一道電光盈繞在$N身上，形成$n \n" + NOR);
     set("unwield_msg", HIR + "$N臉上恨意消退，將$n往天空一拋，$n化為電光隱沒於天際\n" + NOR);
     set("no_sell",1);
     set("no_drop",1);
     set("no_auc",1);
   }
...
void init()
{
   ...
   {
    if(this_player()->query("id") == "luner"  || this_player()->query("id") =="wataru")
    {
       add_action("feed_dag","discipline");
       add_action("target_absorb","absorb");
       add_action("rename_dag","rename");
       add_action("verify_pet","skyhate_data");
...
{//hit_ob的BUG，第一個傳入物件代表被害者，第二個不清楚
  ...
  if( user->is_fighting() && query("equipped") )
  {
	...
	{//為了擴展性，例如還可以加入火之屬性與水之屬性
	 ...
	 case 2:
	 {
    	  message_vision(this_object()->name()+HBMAG + "被戰鬥的氣勁牽引突然光芒一現,幻化成九龍凌空於身！" + NOR);
...
write(@HELP
天恨可以隨著你養育他的方法而成長，具有光明與黑暗兩系的力量。
兩種力量並不相容，所以當其中一方的力量增強，另一方的力量必然下降。
所以，想讓天恨同時具有兩種屬性是不可能的。
天恨在光系的力量是以回復自身為主，暗系的力量是以傷害對手為主。
因此，光系的力量必須以自身的內力去培養，
