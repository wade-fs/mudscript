...
void create()
{
   ...
   {
     set("long",@LONG
想不到在你面前的竟然是傳說中的神兵--莫邪。由劍身上傳來神秘光彩，
令你感到無限的威力，據說此把神兵在近年來已經落入gendor的手中，
為其所有，其他人並無法操作這一把武器。

     下列是你可以對他下的命令，詳細內容請打help moshe
     鍛鍊(discipline)      格式 discipline           ：以自身力量使光系力量成長
     吸收(absorb)     	格式 absorb <target>  ：吸收對方使暗黑系力量成長
     命名(rename)   	格式 rename <name> ：為武器改名
     資料(dag_data)       格式 moshe_data           ：察看武器目前的水準
LONG);
     set("unit","把");
     set("value",0);
     set("material", "steel");
     set("wield_msg", HIM + "$N無生無息地把$n由腰上劍鞘拔出，感到陣陣的悲泣。\n" + NOR);
     set("unwield_msg", HIM + "$N無生無息地把$n插入腰上劍鞘，恢復一般的平靜。\n" + NOR);
     set("no_sell",1);
     set("no_drop",1);
     set("no_auc",1);
   }
...
void init()
{
   ...
   {
    if(this_player()->query("id") == "gendor"  || this_player()->query("id") =="wataru")
    {
       add_action("feed_dag","discipline");
       add_action("target_absorb","absorb");
       add_action("rename_dag","rename");
       add_action("verify_pet","moshe_data");
...
int target_absorb(string str)
{
	...
	if(this_player()->query("bellicosity")< 5)
...
int rename_dag(string arg)
{
   ...
   arg = trans_color(arg);
   this_object()->set_name(arg + NOR,({"moshe"}) );
...
write(@HELP
莫邪可以隨著你養育他的方法而成長，具有光明與黑暗兩系的力量。
兩種力量並不相容，所以當其中一方的力量增強，另一方的力量必然下降。
所以，想讓莫邪同時具有兩種屬性是不可能的。
莫邪在光系的力量是以回復自身為主，暗系的力量是以傷害對手為主。
因此，光系的力量必須以自身的內力去培養，
...
int query_autoload()
{
    if (this_player()->query("id") == "gendor")
        return 1;
}
