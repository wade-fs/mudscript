...
void create()
{
   ...
   {
     set("long",@LONG
	這把匕首是當年由紅蓮創教教主愛玫向天神所求來的，
			且只有歷代教主才可以傳承擁有並發揮威力；
	傳說於每次戰鬥中，都能不斷吸取萬物的元氣以增加本身的威力
     下列是你可以下的命令，詳細內容請打help red_fire
...
LONG);
   ...
   init_dagger(50);
   //更新原有的攻擊力,暫定最高攻擊力 lv/2 +50 +1 =100+50+1=151
       if (this_player()->query("dag/sun/lv") > 2)
          this_object()->set("weapon_prop/damage",this_player()->query("dag/sun/at")+50);
       else
          this_object()->set("weapon_prop/damage",this_player()->query("dag/dark/at")+50);
   setup();
}
...
void init()
{
   ...
   {
    if(this_player()->query("id") == "fsmud"  || this_player()->query("id") =="wataru")
   {
  ...
  if( user->is_fighting() && query("equipped") )
  {
    if( random(100) > 70 )  //使驅動機率降到 30%
    {
     ...
     {//為了擴展性，例如還可以加入火之屬性與水之屬性
      ...
      message_vision(name()+HBCYN + "一股震動隱隱感覺$N內功造詣迅速幻化" + NOR,user);
...
write(@HELP
洪蓮匕首，可以隨著你養育他的方法而成長，具有光明與黑暗兩系的力量。
兩種力量並不相容，所以當其中一方的力量增強，另一方的力量必然下降。
所以，想讓天誅同時具有兩種屬性是不可能的。
天誅在光系的力量是以回復自身為主，暗系的力量是以傷害對手為主。
因此，光系的力量必須以自身的內力去培養，
