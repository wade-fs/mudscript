...
void create()
{
   ...
   set_name(HIC"風花雪月"NOR,({ "snow-finger"}) );
   set_weight(3500);
   if( clonep() )
   ...
   else 
   {
     set("long",@LONG
這把風花雪月是常年存放在開滿花的雪地裡
         傳說只有和它志同道合的武者 , 才能夠完全的擁有它

     下列是你可以對他下的命令，詳細內容請打help snow-finger
     鍛鍊(discipline)      格式 discipline           ：以自身力量使光系力量成長
     吸收(absorb)     	格式 absorb <target>  ：吸收對方使暗黑系力量成長
     命名(rename)   	格式 rename <name> ：為武器改名
     資料(dag_data)       格式 finger_data           ：察看武器目前的水準
LONG);
     set("unit","對");
     set("value",0);
     set("material", "steel");
     set("wield_msg", HIW"$N四週忽然飄散著花朵 , $n隨著光茫出現 \n"NOR);
     set("unwield_msg", HIW"$n隨著片片雪花 , 隱沒於風中\n"NOR);
     set("no_sell",1);
     set("no_drop",1);
     set("no_auc",1);
   }
...
write(@HELP
這把風花雪月可以隨著你養育他的方法而成長，具有光明與黑暗兩系的力量。
兩種力量並不相容，所以當其中一方的力量增強，另一方的力量必然下降。
所以，想風花雪月同時具有兩種屬性是不可能的。
風花雪月在光系的力量是以回復自身為主，暗系的力量是以傷害對手為主。
因此，光系的力量必須以自身的內力去培養，
