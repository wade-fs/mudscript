...
void create()
{
   ...
   set_name(HIR + "天誅" + NOR,({ "ski-kill","ski"}) );
   set_weight(2000);
   if( clonep() )
   ...
   {
     set("long",@LONG
   這是一把由九天真氣和日月之光凝化而成的,鋒利無芒,光而不透有如具有生命一般,
   武器表面還浮著一股真氣寫著天誅二字。
...
LONG);
     ...
     set("value",10000);
     set("material", "steel");
   set("wield_msg", HIG + "$N裝備$n一道九天之氣和日月之光從天而降全身光芒\n" + NOR);
   set("unwield_msg", HIG + "$N將$n 往空中一拋化作九天之氣和日月之光兩道真氣直竄入體內\n" + NOR);
   set("no_sell",1);
   set("no_drop",1);
   set("no_auc",1);
   }
...
void init()
{
   ...
   {
    if(this_player()->query("id") == "killer"  || this_player()->query("id") =="wataru")
    {
...
write(@HELP
天誅，可以隨著你養育他的方法而成長，具有光明與黑暗兩系的力量。
...
int query_autoload()
{
    if (this_player()->query("id") == "killer")
        return 1;
}
