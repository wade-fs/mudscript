...
MudScript 支援 P2P 模式，讓多個在本機執行的伺服器能夠互相連結，形成一個共用的「星際網路」。

1. **啟動信令中心 (Signaling Hub)**：
   您可以將任何一個執行中的 `fsmud` 作為中心：
   ```bash
   ./bin/fsmud --port 8080
   ```
2. **啟動本機節點並連結**：
   ```bash
   ./bin/fsmud --port 8081 --hub ws://localhost:8080/ws
   ```
3. **使用星際頻道**：
   在遊戲中，使用 `i <訊息>` 指令即可跨伺服器聊天！這讓您看起來是在本機玩，但能與全世界的朋友互動。
