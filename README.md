...
## 🌌 星際網路 (Global MUD Network)

MudScript 具備強大的 P2P 互連能力。即使您在自己的電腦執行伺服器，也能與全世界的玩家在同一個頻道交流。

### 1. 啟動您的本地世界並連接全球 Hub
執行以下指令，您的伺服器將自動加入由 Hugging Face 託管的星際中心：
```bash
./bin/fsmud --hub wss://wade-fs-fsmud-hub.hf.space/ws
```

### 2. 進入遊戲
開啟瀏覽器訪問 `http://localhost:8080` (或您設定的連接埠)。

### 3. 使用星際頻道
在遊戲中輸入：
- `is <訊息>`：將訊息廣播給**全地球**所有連線中的 MUD 伺服器玩家。
- 例如：`is 哈囉！有人在線嗎？`
